import { createObservationAccess } from '@agent-core/runtime';
...
const sessionBinding = Object.freeze({
  schemaId: 'agent-core.tests/read-observed-facts',
  schemaVersion: 1,
  subject: Object.freeze({ application: 'read-observed-facts' })
});
...
test('every built-in read tool derives observedFacts from its persisted ToolScope', async () => {
  ...
  const artifact = await artifacts.store({
    label: 'fixture',
    content: new TextEncoder().encode('artifact body'),
...
test('read_files emits per-file success and failure observedFacts and failed searches report failure', async () => {
  ...
  const search = await invokeToolCall(
    jsonToolCall('search_text', { query: '[', mode: 'matches' }),
    [searchTextTool],
    context
  );
...
test('read-tool observedFacts reaches durable observations, session context, prompt selection, and check input', async () => {
  ...
  const session = await sessions.create({
    provider: 'observedFacts-provider',
  ...
  const provider = {
    ...
    async describeModel() {
      return {
        ...
        limits: { contextTokens: 16_000, outputTokens: 1_000 },
          ...
          toolCalls: [
            {
              ...
              type: 'function',
              name: 'list_directory',
              input: { kind: 'json', value: { depth: 1 } }
            }
          ]
        };
      return {
        content: 'observedFacts retained',
        model: request.model,
        provider: this.id,
  ...
  const runtime = new AgentRuntime({ maxOutputTokens: 64,
    ...
    toolBoundary: { authorizationPolicyId: 'tests/read-observed-facts@1', executionTargetId: root },
    toolPolicy: { allowedRisks: ['read'] },
    toolContext: { services }
  });
  const result = await runtime.run({ task: 'inspect the workspace' }).result;
  assert.equal(result.state, 'ended');
  const checkedFacts = await createObservationAccess({
    events,
    artifacts,
    runId: result.terminal.runId
  }).observedFacts.read({ limit: 100 });
  ...
  assert.equal(
    checkedFacts.items.some((item) => item.toolName === 'list_directory' && item.action === 'list'),
    true
  );
