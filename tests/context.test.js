...
test('active image count, byte, and token budgets remove older images deterministically', () => {
  ...
  for (const scenario of scenarios) {
    const manager = new ModelWindow(new CompleteRequestEstimator(), scenario.limits);
...
test('several images in one result obey the same global newest-first budget', () => {
  const manager = new ModelWindow(new CompleteRequestEstimator(), { maxCount: 2, maxBytes: 100, maxEstimatedTokens: 10_000 });
...
test('ModelWindow retains queryable observed facts without injecting a second prompt digest', () => {
  const manager = new ModelWindow();
  manager.recordToolResult({
    turnIndex: 1,
  ...
  const facts = manager.selectObservedFacts(4000);
  assert.equal(assembly.prompt.observedFacts, undefined);
  assert.equal(facts.records.length, 2);
  assert.deepEqual(facts.records.map((record) => record.action), ['list', 'execute']);
  assert.equal(facts.records.some((record) => record.action === 'read'), false);
  assert.equal(assembly.estimate.observedFactTokens, 0);
});
...
test('selected source records preserve complete conversation independently of result presentation reduction', () => {
  ...
  window.recordSourceItem('source:user:1', {role:'user',content:original});
  window.reduceHistoryForPromptPressure({modelProfile,maxHistoryTokens:10});
  const assembled = new ModelRequestAssembler().assemble({window,task:'new task',instructions:[],tools:[],modelProfile,maxPromptTokens:16000});
  ...
  assert.throws(()=>window.recordSourceItem('source:user:1',{role:'user',content:'mutated'}),/immutable content/);
 ...
 test('all exact tool arguments survive presentation pressure, including arguments larger than display output', () => {
  ...
  window.recordModelOutput({turnIndex:1,content:'',toolCalls:[{id:'large',type:'function',name:'inspect',input:{kind:'json',value:{value}}}]});
  window.recordToolResult({turnIndex:1,toolName:'inspect',toolCallType:'function',callId:'large',immediateContent:'large result '.repeat(5000),retainedContent:'retained observation'});
  window.reduceHistoryForPromptPressure({modelProfile,maxHistoryTokens:10});
  ...
  assert.ok(history.estimatedTokens>10000);
});
