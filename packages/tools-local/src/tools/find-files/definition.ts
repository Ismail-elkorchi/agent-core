...
export const findFilesTool = defineTool({
  ...
  async invoke(input, context) {
    ...
    const scope = {
      ...
      filters: { patterns: input.patterns, exclude: input.exclude, type: input.type },
      limits: { hostMaximumDepth: selected.hostMaximumDepth },
    ...
    return {
      ...
      observedFacts: builtInObservedFacts(
        'search',
        scope,
        `Found ${String(output.counts.returned)} matching paths.`
      ),
      output
    };
  }
