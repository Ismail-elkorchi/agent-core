...
export async function readFiles(
  ...
  for (const [index, request] of input.files.entries()) {
    ...
    if (index >= limits.maxFiles) {
      failures.push({
        path: request.path,
        reason: 'batch_file_limit',
        message: 'The host accepts at most ' + String(limits.maxFiles) + ' files in one batch.'
      });
      continue;
    }
    if (remainingBytes <= 0) {
      failures.push({
        path: request.path,
        reason: 'batch_byte_limit',
        message: 'The global read_files byte budget is exhausted.'
      });
      continue;
    }
    ...
    const result = await readRange(
      ...
      remainingBytes < limits.maxBytesPerFile,
      context
    );
    if (result.ok) {
  ...
  const scope = {
    resources: [...new Set(input.files.map((file) => fileScope(file.path)))],
    coverage,
    limits: {
...
function readFileObservedFacts(
  files: readonly ReadFileResult[],
  failures: readonly ReadFileFailure[],
  scope: {
          ...
          ...(file.lineCount > 0
            ? {
                range: {
                  ...
                  end: file.startLine + file.lineCount - 1
                }
              }
            : {}),
...
async function readRange(
  ...
  try {
    ...
    return failure(
      displayPath,
      nodeCode(error) === 'ENOENT' ? 'not_found' : 'unreadable',
      errorMessage(error)
    );
  }
  try {
    ...
    await context.emitProgress?.({
      type: 'status',
      stage: 'file_reading',
      message: `Reading stable file ${displayPath}.`
    });
        ...
        if (currentLine >= startLine && selectedLines < lineCount) {
          if (selectedBytes + segment.byteLength > maxBytes) {
            ...
            return failure(
              ...
              reason,
              reason === 'batch_byte_limit'
                ? 'The global read_files byte budget is exhausted.'
                : 'Requested range exceeds the host byte limit for one result.'
            );
          }
    ...
    if (fileHasUnterminatedLine && currentLine >= startLine && selectedLines < lineCount)
      selectedLines += 1;
    const totalLines =
      fileBytes === 0
        ? 0
    ...
    if (fileBytes === 0 && startLine === 1) {
      ...
      return {
        ...
        value: {
          ...
          eof: true,
          truncated: false,
          rangeSha256: emptySha256,
          fullFileSha256: emptySha256,
          newlineConvention: 'none',
    ...
    if (startLine > totalLines)
      return failure(
        ...
        'Requested start line ' +
          String(startLine) +
          ' is after EOF at line ' +
          String(totalLines) +
          '.'
      );
    if (binary)
      return failure(displayPath, 'binary', 'File contains binary data: ' + requestedPath);
    if (!validUtf8)
