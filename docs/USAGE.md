...
Use package root exports for platform-neutral contracts. Filesystem repositories are exposed only through `@agent-core/runtime/node` and `@agent-core/evidence/node`. Node and workspace tool implementations are exposed by `@agent-core/tools-local`.
