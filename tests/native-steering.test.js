...
test('AgentRuntime delivers native steering during an active provider response',async()=>{
 ...
 const runtime=new AgentRuntime({ maxOutputTokens: 64,provider,model:'native',repositories:{events},toolBoundary:{authorizationPolicyId:'none',executionTargetId:'none'},onProgress:event=>{if(event.type==='assistant.delta'&&event.accumulated==='working')handle.injectSteering({instruction:'Apply this correction now.'});}});
