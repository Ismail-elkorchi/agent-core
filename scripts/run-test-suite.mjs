...
async function run() {
  ...
  // This scalability case assembles 1,000 complete prompts. Run it without competing
  // worker processes so its wall-clock bound measures the harness, not suite contention.
  const longSession = files.filter((file) => file === path.join(root, 'tests/long-session-inputs.test.js'));
  for (const group of [files.filter((file) => !longSession.includes(file)), longSession]) {
    if (group.length === 0) continue;
    const child = spawn(process.execPath, ['--test', ...group], { cwd: root, stdio: 'inherit' });
    const code = await new Promise((resolve, reject) => {
      child.once('error', reject);
      child.once('exit', (value, signal) =>
        signal ? reject(new Error(`Ordinary test process ended with ${signal}.`)) : resolve(value ?? 1)
      );
    });
    if (code !== 0) {
