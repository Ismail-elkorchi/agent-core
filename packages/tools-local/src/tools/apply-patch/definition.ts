...
export const applyPatchTool = defineTool({
  ...
  async canonicalizeInput(input, context): Promise<CanonicalApplyPatchInput> {
    ...
    await context.emitProgress?.({
      type: 'status',
      stage: 'patch_parsing',
      message: 'Parsing patch.'
    });
    try {
      tree = parseApplyPatch(input.patch, { maxPatchBytes: limits.maxPatchBytes });
    } catch (error) {
    ...
    if (tree.operations.length > limits.maxOperations) {
      throw new ToolInputError(
        `Patch contains ${String(tree.operations.length)} operations; the host limit is ${String(limits.maxOperations)}.`
      );
    }
    await context.emitProgress?.({
      type: 'status',
      stage: 'patch_parsed',
      message: 'Patch parsed.',
      completed: tree.operations.length,
      total: tree.operations.length
    });
...
function patchPaths(tree: ParsedApplyPatch): string[] {
  return tree.operations.flatMap((operation) =>
