...
async function codexFixture(t, { expires = 4102444800 } = {}) {
  ...
  const credentials = JSON.stringify({ auth_mode: 'chatgpt', tokens: { access_token: token, account_id: 'private-account-id', id_token: 'private-id-token', refresh_token: 'private-refresh-token' }, last_refresh: '2026-09-07T00:00:00Z' });
  await writeFile(authFile, credentials, { mode: 0o400 });
  const before = await stat(authFile, { bigint: true });
  return { directory, token, credentials, authFile,
    options: { provider: 'codex', model: 'gpt-5.6', endpoint: codexEndpoint, modelVersion: 'fixture-deployment-v1', codexAuthFile: authFile },
    ...
    assertPrivate(text) {
      for (const secret of [token, 'private-account-id', 'private-id-token', 'private-refresh-token', authFile]) assert.ok(!text.includes(secret), 'Private auth material must stay out of reports and diagnostics.');
    }
  };
}
...
test('Codex flags are explicit, dry mode does not read auth, and subscription endpoints are restricted', async (t) => {
  ...
  t.mock.method(globalThis, 'fetch', () => { assert.fail('No network in dry/capabilities mode.'); });
  const argv = ['--provider', 'codex', '--model', 'gpt-5.6', '--endpoint', codexEndpoint, '--model-version', 'fixture-deployment-v1', '--codex-auth-file', fixture.authFile];
  const options = parseArguments(argv, { CONTEXT_EVAL_MODEL: 'overridden', CONTEXT_EVAL_API_KEY: 'unused-api-secret' });
  assert.equal(options.model, 'gpt-5.6');
  assert.equal(options.codexAuthFile, fixture.authFile);
  const dry = await evaluateContextPolicies({ ...options, codexAuthFile: path.join(fixture.directory, 'does-not-exist') }, {});
  ...
  for (const endpoint of ['https://example.invalid/codex', 'http://chatgpt.com/backend-api', 'https://chatgpt.com/backend-api/private', 'https://chatgpt.com/backend-api/codex?key=secret']) {
    assert.throws(() => parseArguments(['--provider', 'codex', '--endpoint', endpoint], {}));
  }
  const absent = await evaluateContextPolicies({ ...fixture.options, mode: 'live', codexAuthFile: undefined }, { OPENAI_API_KEY: 'ignored' });
  assert.match(absent.availability[0].reason, /no implicit credential discovery/u);
  const unknown = await evaluateContextPolicies({ ...fixture.options, mode: 'capabilities', model: 'invented-codex-model' }, {});
  assert.match(unknown.availability[0].reason, /model_unavailable/u);
});

test('bounded Codex generation uses public provider/auth composition without auth writes or secret reporting', { timeout: 60_000 }, async (t) => {
  const fixture = await codexFixture(t);
  const requests = [];
  const logs = [];
  ...
  t.mock.method(globalThis, 'fetch', (url, init) => {
    ...
    const response = { id: `fixture-response-${requests.length}`, status: 'completed', output: [{ type: 'message', role: 'assistant', content: [{ type: 'output_text', text: 'ACK' }] }], usage: { input_tokens: 20, output_tokens: 4, total_tokens: 24 } };
    return Promise.resolve(new Response(`data: ${JSON.stringify({ type: 'response.completed', response })}\n\n`, { headers: { 'content-type': 'text/event-stream' } }));
  });
  const report = await evaluateContextPolicies({ ...fixture.options, mode: 'live', trials: 1, delay: 1, policies: ['retained-history'], maxTotalInvocations: 1 }, {});
  ...
  fixture.assertPrivate(JSON.stringify(report));
  const notes = await evaluateContextPolicies({ ...fixture.options, mode: 'live', trials: 1, delay: 1, transitionEvery: 2, policies: ['notes-retrieval'], maxTotalInvocations: 4 }, {});
...
test('Codex auth and provider failures keep file contents and private errors out of CLI output', async (t) => {
  ...
  const args = ['scripts/evaluate-context-policies.mjs', '--mode', 'live', '--provider', 'codex', '--model', 'gpt-5.6', '--model-version', 'fixture', '--endpoint', codexEndpoint, '--codex-auth-file', fixture.authFile, '--output', output];
  ...
  const invalid = await evaluateContextPolicies({ ...valid.options, mode: 'capabilities', codexAuthFile: malformedFile }, {});
...
test('dry configuration never calls a provider and never reports environment credentials', async () => {
  ...
  const env = { CONTEXT_EVAL_PROVIDER: 'openai', CONTEXT_EVAL_MODEL: 'unresolved-model', CONTEXT_EVAL_ENDPOINT: 'https://example.invalid/v1', CONTEXT_EVAL_API_KEY: secret };
  ...
  const unavailable = await evaluateContextPolicies({ mode: 'live', provider: 'openai', model: 'example', endpoint: 'https://example.invalid/v1' }, {});
  assert.ok(unavailable.availability.every((item) => item.status === 'unavailable' && item.reason.includes('Missing credentials')));
  assert.deepEqual(unavailable.trials, []);
  for (const provider of ['constructor', '__proto__']) {
    const unsupported = await evaluateContextPolicies({ mode: 'capabilities', provider, model: 'model', endpoint: 'https://example.invalid' }, {});
...
test('runner rejects unbounded/ambiguous inputs and credential-bearing endpoints', () => {
  for (const argv of [
    ['--trials', 'Infinity'], ['--trials', '0'], ['--max-total-invocations', '501'],
    ['--timeout-ms', '0'], ['--policies', 'notes-retrieval,notes-retrieval'],
    ['--mode', 'unknown'], ['--policies', 'mystery'], ['--trials'], ['--secret', 'value']
  ]) assert.throws(() => parseArguments(argv, {}));
  for (const endpoint of ['https://user:password@example.invalid', 'https://example.invalid?api_key=secret', 'file:///tmp/provider']) {
...
test('comparison gates cannot turn low prompt usage or missing costs into quality evidence', () => {
  ...
  const row = (policy, trial, success, promptTokens) => ({ policy, workload: 'gate-arithmetic', trial, success,
    mechanicalCoverage: 'observed-trial', mechanical: { inputLoss: 0, unauthorizedScopeExpansion: 0, orphanToolResults: 0, duplicateSettlements: 0 },
    metrics: { promptTokens, completionTokens: 10, latencyMs: 1, knownCosts: { USD: 0.01 }, costStatus: 'known' } });
  const trials = Array.from({ length: 20 }, (_, index) => [row('retained-history', index, true, 1000), row('notes-retrieval', index, false, 10)]).flat();
  ...
  assert.ok(summarizeTrials(trials, gates, 'deterministic-simulation').every((item) => item.qualityGate === 'not-measured'));
  const unknownCosts = trials.map((trial) => ({ ...trial, success: true, metrics: { ...trial.metrics, costStatus: 'unknown-or-partial' } }));
  assert.ok(summarizeTrials(unknownCosts, gates).every((item) => item.qualityGate === 'inconclusive' && item.meanCostUSD === null));
  unknownCosts[0].mechanical = { ...unknownCosts[0].mechanical, duplicateSettlements: 1 };
  assert.equal(summarizeTrials(unknownCosts, gates).find((item) => item.policy === 'retained-history').qualityGate, 'failed-mechanical');
  const estimatedCosts = trials.map((trial) => ({ ...trial, metrics: { ...trial.metrics, tokenUsageSource: 'includes-estimates' } }));
...
test('CLI writes a caller-selected report and preserves an existing output file', async (t) => {
  ...
  const env = { ...process.env, CONTEXT_EVAL_PROVIDER: 'openai', CONTEXT_EVAL_MODEL: 'model', CONTEXT_EVAL_ENDPOINT: 'https://example.invalid/v1', CONTEXT_EVAL_API_KEY: 'transport-secret' };
...
test('deterministic policy trials execute Core inference, context transitions, notes and authorized retrieval', { timeout: 120_000 }, async () => {
  const report = await evaluateContextPolicies({ mode: 'simulation', trials: 2, delay: 1, transitionEvery: 2, maxTotalInvocations: 200 });
  ...
  assert.ok(report.configuration.source.dirty === null || typeof report.configuration.source.dirty === 'boolean');
  assert.equal(report.trials.length, 16);
  assert.deepEqual(report.availability.map((item) => item.policy), POLICIES);
  ...
  for (const trial of report.trials) {
    ...
    assert.equal(trial.metrics.retrievalCalls > 0, ['history-retrieval', 'notes-retrieval'].includes(trial.policy));
    assert.equal(trial.metrics.retrievalFailures, 0);
  }
  assert.equal(report.totalInvocations, report.trials.reduce((sum, trial) => sum + trial.metrics.invocations, 0));
  assert.ok(report.summary.every((item) => item.sampleSize === 2 && item.qualityGate === 'not-measured' && item.meanCostUSD === null));
});

test('a whole-comparison invocation ceiling stops work and retains unmeasured checkpoints as failures', { timeout: 60_000 }, async () => {
  const report = await evaluateContextPolicies({ mode: 'simulation', trials: 1, delay: 1, policies: ['retained-history'], maxTotalInvocations: 1 });
  assert.equal(report.totalInvocations, 1);
  assert.ok(report.trials.every((trial) => trial.status === 'budget-exhausted' && !trial.success));
  assert.ok(report.trials.every((trial) => trial.checkpoints.length === 2 && trial.checkpoints.every((point) => point.unavailable && !point.success)));
});

test('primary and auxiliary costs use durable public pricing, including cache rates and explicit partial costs', { timeout: 60_000 }, async (t) => {
  const { MemorySimulationProvider, simulationProfile } = await import('./fixtures/context-policies/simulation.mjs');
  ...
  const records = [];
  let pricing;
  t.mock.method(MemorySimulationProvider.prototype, 'describeModel', async () => ({ ...simulationProfile, pricing }));
  t.mock.method(MemorySimulationProvider.prototype, 'complete', async function (request) {
    return { ...await complete.call(this, request), usage: { promptTokens: 100, completionTokens: 10, totalTokens: 110, cacheReadTokens: 40 } };
  });
  ...
  for (const known of [true, false]) {
    ...
    const report = await evaluateContextPolicies({ mode: 'simulation', trials: 1, delay: 1, transitionEvery: 2, policies: ['notes-retrieval'], maxTotalInvocations: 100 });
    for (const trial of report.trials) {
      assert.equal(trial.status, 'completed', trial.failure);
      assert.ok(trial.metrics.noteWrites > 0);
      const settlements = records.filter(({ ownerId, event }) => trial.runIds.includes(ownerId) && event.type === 'inference.settled').map(({ event }) => event);
      ...
      const pricePerInvocation = known ? 0.00022 : 0.00020;
...
test('deterministic native transform settles and replays through the shared inference owner budget', async () => {
  const { MemorySimulationProvider, simulationProfile } = await import('./fixtures/context-policies/simulation.mjs');
  const { InferenceService, InMemoryInferenceRepository, InferenceBudgetExceededError } = await import('@agent-core/runtime');
  ...
  const options = { provider, repository, artifacts, budget: { maxInvocations: 2, maxPromptTokens: 10000, maxCompletionTokens: 256 } };
  const service = new InferenceService(options);
  const request = { model: simulationProfile.id, maxOutputTokens: 64, messages: [{ role: 'user', content: 'STATE[000] {"code":"HERON"}' }] };
  await service.invoke({ invocationId: 'primary', ownerId: 'native-owner', purpose: 'agent-step', request });
  const input = { invocationId: 'native-transform', ownerId: 'native-owner', purpose: 'context-transition', transformId: 'simulation.compact', request };
  ...
  assert.deepEqual([...owner.invocations.values()].map((item) => item.start.operation), ['generation', 'context_transform']);
  assert.ok([...owner.invocations.values()].every((item) => item.settlement));
  await assert.rejects(service.invoke({ invocationId: 'too-many', ownerId: 'native-owner', purpose: 'agent-step', request }), InferenceBudgetExceededError);
  assert.equal(provider.calls.length, 1);
});

test('native policy commits protected transform references and delivers their exact output to the next runtime', { timeout: 60_000 }, async (t) => {
  ...
  t.mock.method(MemorySimulationProvider.prototype, 'transformContextCompiled', async function (transformId, compiled) {
    const result = await transform.call(this, transformId, compiled);
    steps.push({ kind: 'transform', compiled, result });
    return result;
  });
  t.mock.method(InMemorySessionRepository.prototype, 'commitContextTransition', async function (session, input) {
  ...
  const report = await evaluateContextPolicies({ mode: 'simulation', trials: 1, delay: 1, transitionEvery: 2, policies: ['provider-native'], maxTotalInvocations: 100 });
  ...
  assert.equal(invocations.filter((item) => item.operation === 'context_transform').length, transforms.length);
  for (const step of transforms) {
    const window = windows.find((item) => item.selection.providerState.transformId === step.result.transformId);
    ...
    const invocation = invocations.find((item) => item.invocationId === window.selection.providerState.invocationId);
    ...
    assert.ok(next);
    for (const item of step.result.input) assert.ok(next.request.messages.some((message) => JSON.stringify(message) === JSON.stringify(item)));
...
test('comparison ceiling prevents auxiliary native dispatch and leaves the previous window active', async () => {
  const report = await evaluateContextPolicies({ mode: 'simulation', trials: 1, delay: 1, transitionEvery: 1, policies: ['provider-native'], maxTotalInvocations: 1 });
  assert.equal(report.totalInvocations, 1);
  assert.ok(report.trials.every((trial) => trial.status === 'budget-exhausted' && trial.budgetStop === 'comparison_invocations'));
  assert.equal(report.trials[0].metrics.settledInvocations, 1);
  assert.ok(report.trials.every((trial) => trial.metrics.uncertainInvocations === 0 && trial.metrics.nativeTransforms === 0 && trial.metrics.contextTransitions === 0));
});

test('OpenAI native policy counts opaque state and dispatches the committed window through the public adapter', { timeout: 60_000 }, async (t) => {
  ...
  t.mock.method(ContextService.prototype, 'transition', async function (...args) {
    try { return await transition.apply(this, args); }
  ...
  t.mock.method(globalThis, 'fetch', async (url, init) => {
    ...
    if (pathname === '/v1/responses/compact') {
      ...
      return Response.json({ id: 'compact-fixture', output: [{ type: 'compaction', encrypted_content: opaque }], usage: { input_tokens: 60, output_tokens: 12, total_tokens: 72 } });
    }
    assert.equal(pathname, '/v1/responses');
    const response = { id: `response-fixture-${requests.length}`, status: 'completed', output: [{ type: 'message', role: 'assistant', content: [{ type: 'output_text', text: 'ACK' }] }], usage: { input_tokens: 60, output_tokens: 4, total_tokens: 64 } };
    return new Response(`data: ${JSON.stringify({ type: 'response.completed', response })}\n\n`, { headers: { 'content-type': 'text/event-stream' } });
  });
  const report = await evaluateContextPolicies({ mode: 'live', provider: 'openai', model: 'gpt-6-astra', modelVersion: 'fixture-deployment', endpoint: 'https://api.openai.com/v1', trials: 1, delay: 1, transitionEvery: 2, policies: ['provider-native'], maxTotalInvocations: 4 }, { CONTEXT_EVAL_API_KEY: 'fake-openai-key' });
  assert.equal(report.totalInvocations, 4, failures.join('\n') || JSON.stringify(report.trials.map(({ status, failure }) => ({ status, failure }))));
  ...
  assert.equal(report.accounting.requests, requests.filter((item) => item.pathname.endsWith('/input_tokens')).length);
  assert.ok(report.accounting.requests > 0 && report.accounting.requests <= report.accounting.maxRequests);
  ...
  assert.ok(generations.at(-1).body.input.some((item) => item.type === 'compaction' && item.encrypted_content === opaque));
  assert.ok(requests.some((item) => item.pathname.endsWith('/input_tokens') && JSON.stringify(item.body).includes(opaque)));
