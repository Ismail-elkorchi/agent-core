...
const mixedContentTool = defineTool({
  name: 'mixed_content',
  implementationId: 'tests/mixed-content@1',
  description: 'Mixed content fixture.',
  schema: z.strictObject({}),
  outputSchema: z.strictObject({ status: z.literal('complete') }),
...
test('read_artifact image content is retained for multimodal models and represented as metadata for text-only models', async () => {
  ...
  const observation = await invokeToolCall(
    jsonToolCall('read_artifact', { artifactId: artifact.artifactId, byteCount: artifact.size }),
    [readArtifactTool],
    {
  ...
  assert.deepEqual(
    textOnly.metadata.modelContentFilter.convertedToArtifactMetadata.map((ref) => ref.artifactId),
    [artifact.artifactId]
  );
});
...
test('the generic result-content filter preserves supported members of mixed text and image content', async () => {
  ...
  const observation = parseToolObservation(mixedContentTool, {
    ...
    scope: { resources: [`artifacts/${artifact.artifactId}`], coverage: 'complete' },
    content: [
      { type: 'text', text: 'keep this', mediaType: 'text/plain' },
  ...
  assert.deepEqual(
    filtered.content.map((item) => item.type),
    ['text', 'artifact']
  );
