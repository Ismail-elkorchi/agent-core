...
export type EffectRecoveryCapability =
  ...
  | {
      readonly kind: 'buffered_mutation';
...
export interface EffectExposureReservation {
  readonly quantities: readonly EffectExposureQuantity[];
}
...
export type ExternalEffectSettlement =
  | {
      ...
      readonly exposure: EffectExposureSettlement;
    }
  | {
      readonly outcome: 'unknown';
      readonly exposure: Extract<EffectExposureSettlement, { readonly status: 'unknown' }>;
    };

export type EffectClosure =
  | { readonly reason: 'cancelled_before_start' }
  | {
      readonly reason: 'expired' | 'reconciliation_unavailable' | 'unknown_outcome';
      readonly exposure: Extract<EffectExposureSettlement, { readonly status: 'unknown' }>;
    };
...
export type EffectTicketIssueResult =
  | {
      ...
      readonly state: Extract<EffectExecutionState, { readonly phase: 'ticket_issued' }>;
    }
...
export type EffectStartResult =
  | {
      ...
      readonly state: Extract<EffectExecutionState, { readonly phase: 'started' }>;
    }
  | {
...
export const NO_EFFECT_EXPOSURE: EffectExposureReservation = Object.freeze({
...
export function decodeEffectRecoveryCapability(value: unknown): EffectRecoveryCapability {
  ...
  if (record.kind === 'preconditioned_reexecution') {
    if (!Array.isArray(record.preconditions) || record.preconditions.length === 0)
      throw new TypeError('Preconditioned re-execution requires at least one resource precondition.');
    const preconditions = record.preconditions.map((candidate) => {
      const item = object(candidate, 'effect resource precondition');
      return Object.freeze({
        resource: identifier(item.resource, 'resource'),
        validatorId: identifier(item.validatorId, 'validatorId'),
        expectedVersion: identifier(item.expectedVersion, 'expectedVersion')
      });
    });
    const identities = preconditions.map((item) => `${item.resource}\0${item.validatorId}`);
    if (new Set(identities).size !== identities.length)
      throw new TypeError('Effect resource preconditions must be unique by resource and validator.');
    return Object.freeze({
      kind: 'preconditioned_reexecution',
      preconditions: Object.freeze(preconditions)
    });
  }
  if (record.kind === 'queryable')
    return Object.freeze({
      kind: 'queryable',
      service: identifier(record.service, 'service'),
      reconcilerId: identifier(record.reconcilerId, 'reconcilerId'),
      externalExecutionId: identifier(record.externalExecutionId, 'externalExecutionId'),
      expiresAt: record.expiresAt === null ? null : isoTimestamp(record.expiresAt, 'expiresAt')
    });
  if (record.kind === 'idempotency_key')
    return Object.freeze({
      kind: 'idempotency_key',
      service: identifier(record.service, 'service'),
      key: identifier(record.key, 'key'),
      expiresAt: isoTimestamp(record.expiresAt, 'expiresAt')
    });
  if (record.kind === 'buffered_mutation')
    return Object.freeze({
      kind: 'buffered_mutation',
      authority: identifier(record.authority, 'authority'),
      reconcilerId: identifier(record.reconcilerId, 'reconcilerId'),
...
export function decodeExternalEffectIntent(value: unknown): ExternalEffectIntent {
  ...
  return Object.freeze({
    effectId: identifier(record.effectId, 'effectId'),
    ownerId: identifier(record.ownerId, 'ownerId'),
    implementationId: identifier(record.implementationId, 'implementationId'),
    parametersDigest: digest(record.parametersDigest, 'parametersDigest'),
    recovery: decodeEffectRecoveryCapability(record.recovery),
    exposure: decodeEffectExposureReservation(record.exposure)
  });
}
...
export function decodeExternalEffectSettlement(value: unknown): ExternalEffectSettlement {
  ...
  if (record.outcome === 'unknown') {
    if (exposure.status !== 'unknown')
      throw new TypeError('An unknown effect outcome must retain unknown exposure.');
    return Object.freeze({ outcome: 'unknown', exposure });
  }
  if (record.outcome !== 'succeeded' && record.outcome !== 'failed' && record.outcome !== 'cancelled')
    throw new TypeError('Invalid external effect settlement outcome.');
  return Object.freeze({
...
export function decodeEffectExecutionState(value: unknown): EffectExecutionState {
  ...
  if (record.phase === 'settled')
    return Object.freeze({
      ...
      intent,
      settlementPermit,
      settlement: decodeExternalEffectSettlement(record.settlement)
    });
  if (record.phase === 'closed')
    return Object.freeze({
      ...
      intent,
      settlementPermit,
      closure: decodeClosure(record.closure, intent.exposure)
    });
...
export function encodeEffectRecoveryCapability(value: EffectRecoveryCapability): JsonObject {
  ...
  if (value.kind === 'preconditioned_reexecution')
    return Object.freeze({
      kind: value.kind,
      preconditions: Object.freeze(
        value.preconditions.map((item) =>
          Object.freeze({
  ...
  if (value.kind === 'queryable')
    return Object.freeze({
      kind: value.kind,
      service: value.service,
      reconcilerId: value.reconcilerId,
      externalExecutionId: value.externalExecutionId,
      expiresAt: value.expiresAt
    });
  if (value.kind === 'idempotency_key')
    return Object.freeze({
      kind: value.kind,
      service: value.service,
      key: value.key,
...
export function encodeExternalEffectIntent(value: ExternalEffectIntent): JsonObject {
  return Object.freeze({
    ...
    recovery: encodeEffectRecoveryCapability(value.recovery),
    exposure: encodeEffectExposureReservation(value.exposure)
  });
}
...
export function encodeExternalEffectSettlement(value: ExternalEffectSettlement): JsonObject {
  return Object.freeze({
    ...
    ...(value.outcome === 'unknown' ? {} : { resultDigest: value.resultDigest }),
    exposure: encodeExposureSettlement(value.exposure)
  });
}
...
export function encodeEffectExecutionState(value: EffectExecutionState): JsonObject {
  const base = {
    ...
    settlementPermit: Object.freeze({
      permitId: value.settlementPermit.permitId,
      effectId: value.settlementPermit.effectId,
      parametersDigest: value.settlementPermit.parametersDigest
    })
  };
  if (value.phase === 'ticket_issued' || value.phase === 'started')
    return Object.freeze({
      ...base,
      ticket: Object.freeze({
        ticketId: value.ticket.ticketId,
        effectId: value.ticket.effectId,
        parametersDigest: value.ticket.parametersDigest,
        driverGeneration: value.ticket.driverGeneration
      })
    });
  if (value.phase === 'settled')
    return Object.freeze({ ...base, settlement: encodeExternalEffectSettlement(value.settlement) });
...
export function issueEffectStartTicket(input: {
  ...
  if (input.driverGeneration !== input.currentDriverGeneration)
    return Object.freeze({ status: 'rejected', reason: 'stale_driver' });
  const ticket = Object.freeze({
    ticketId,
    effectId: input.intent.effectId,
    parametersDigest: input.intent.parametersDigest,
    driverGeneration: input.driverGeneration
  });
  const settlementPermit = Object.freeze({
    permitId,
    effectId: input.intent.effectId,
    parametersDigest: input.intent.parametersDigest
  });
  return Object.freeze({
    status: 'issued',
    state: Object.freeze({ phase: 'ticket_issued', intent: input.intent, ticket, settlementPermit })
  });
}

export function startExternalEffect(
  state: EffectExecutionState,
  ticket: EffectStartTicket,
  currentDriverGeneration: number
): EffectStartResult {
  generation(currentDriverGeneration, 'currentDriverGeneration');
  if (ticket.driverGeneration !== currentDriverGeneration)
    return Object.freeze({ status: 'rejected', reason: 'stale_driver' });
  if (state.phase === 'settled' || state.phase === 'closed')
    return Object.freeze({ status: 'rejected', reason: 'effect_terminal' });
  if (state.phase === 'started') return Object.freeze({ status: 'rejected', reason: 'ticket_consumed' });
  if (!ticketsEqual(state.ticket, ticket))
    return Object.freeze({ status: 'rejected', reason: 'ticket_mismatch' });
  return Object.freeze({
    status: 'started',
    state: Object.freeze({
      phase: 'started',
      intent: state.intent,
      ticket: state.ticket,
      settlementPermit: state.settlementPermit
    })
  });
}
...
export function settleExternalEffect<Settlement extends ExternalEffectSettlement>(
  ...
  if (state.phase === 'ticket_issued')
    return Object.freeze({ status: 'rejected', reason: 'effect_not_started' });
  if (state.phase === 'closed') return Object.freeze({ status: 'late', state });
  if (state.phase === 'settled')
    return settlementsEqual(state.settlement, settlement)
...
export function closeExternalEffect(
  ...
  if (state.phase === 'started' && reason === 'cancelled_before_start')
    throw new TypeError('A started external effect cannot be closed as cancelled before start.');
  if (state.phase === 'ticket_issued' && reason !== 'cancelled_before_start')
    throw new TypeError('An unstarted external effect has no external outcome to reconcile.');
  const closure: EffectClosure =
...
export function knownEffectExposure(
  quantities: readonly EffectExposureQuantity[]
): EffectExposureSettlement {
  return Object.freeze({ status: 'known', quantities: ownQuantities(quantities) });
}

export function unknownEffectExposure(
  reservation: EffectExposureReservation
): Extract<EffectExposureSettlement, { readonly status: 'unknown' }> {
...
function decodeExposureSettlement(value: unknown): EffectExposureSettlement {
  ...
  if (record.status === 'known')
    return Object.freeze({
      status: 'known',
      quantities: decodeQuantities(record.quantities, 'known effect exposure')
    });
  if (record.status === 'unknown')
    return Object.freeze({
      status: 'unknown',
      reserved: decodeQuantities(record.reserved, 'unknown effect exposure')
    });
...
function decodeStartTicket(value: unknown): EffectStartTicket {
  ...
  return Object.freeze({
    ticketId: identifier(record.ticketId, 'ticketId'),
    effectId: identifier(record.effectId, 'effectId'),
    parametersDigest: digest(record.parametersDigest, 'parametersDigest'),
    driverGeneration
  });
...
export function decodeEffectSettlementPermit(value: unknown): EffectSettlementPermit {
  ...
  return Object.freeze({
    permitId: identifier(record.permitId, 'permitId'),
    effectId: identifier(record.effectId, 'effectId'),
    parametersDigest: digest(record.parametersDigest, 'parametersDigest')
  });
}
...
function decodeClosure(value: unknown, reservation: EffectExposureReservation): EffectClosure {
  ...
  if (
    ...
    record.reason !== 'reconciliation_unavailable' &&
    record.reason !== 'unknown_outcome'
  )
    throw new TypeError('Invalid effect closure reason.');
  const exposure = decodeExposureSettlement(record.exposure);
  if (exposure.status !== 'unknown')
    throw new TypeError('An unresolved effect closure must retain unknown exposure.');
  if (!quantitiesEqual(exposure.reserved, reservation.quantities))
    throw new TypeError('An unresolved effect closure must retain the exact exposure reservation.');
...
function decodeQuantities(value: unknown, label: string): readonly EffectExposureQuantity[] {
  ...
  return ownQuantities(
    value.map((candidate) => {
      const item = object(candidate, `${label} quantity`);
      if (!Number.isSafeInteger(item.amount) || Number(item.amount) < 0)
        throw new TypeError(`${label} amounts must be nonnegative safe integers.`);
...
function ownQuantities(value: readonly EffectExposureQuantity[]): readonly EffectExposureQuantity[] {
  ...
  if (new Set(quantities.map((item) => item.unit)).size !== quantities.length)
    throw new TypeError('Effect exposure units must be unique.');
...
function settlementsEqual(left: ExternalEffectSettlement, right: ExternalEffectSettlement): boolean {
  return (
    JSON.stringify(encodeExternalEffectSettlement(left)) ===
    JSON.stringify(encodeExternalEffectSettlement(right))
  );
}

function quantitiesEqual(
  left: readonly EffectExposureQuantity[],
  right: readonly EffectExposureQuantity[]
): boolean {
...
function assertEffectBinding(
  ...
  value: { readonly effectId: string; readonly parametersDigest: string },
  label: string
): void {
  if (intent.effectId !== value.effectId || intent.parametersDigest !== value.parametersDigest)
    throw new TypeError(`Effect ${label} does not match its intent.`);
}
...
export { executeEffectLifecycle, type EffectLifecycle } from './lifecycle.js';
