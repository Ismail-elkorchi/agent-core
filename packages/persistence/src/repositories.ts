import { canonicalJsonString, parseJsonObject, type JsonObject } from '@agent-core/json';
...
export class JsonlEventRepository<TEvent extends TypedEvent> implements EventRepository<TEvent> {
            ...
            if (existing) {
              if (existing.eventDigest !== hashJson(encodedEvent)) {
                throw new PersistenceConflictError(
                  `Idempotency key ${options.idempotencyKey} already identifies a different event.`
                );
              }
  ...
  appendConditional(
    ...
    return this.enqueue(runId, async () => {
      ...
      const encodedEvent = this.codec.encode(event);
      try {
        return await withPersistenceFileLock(
          this.filePath(runId),
          this.lockTimeoutMs,
          this.staleLockMs,
          async (assertOwned) => {
  ...
  tail(runId: string): Promise<EventLedgerTail> {
    return this.enqueue(runId, async () => {
      try {
        return tailFromEncoded((await this.refreshIndex(runId, false)).tail);
      } catch (error) {
        if (nodeCode(error) === 'ENOENT') return tailFromEncoded(undefined);
        throw error;
      }
    });
  }
  ...
  latestOfType(runId: string, type: TEvent['type']): Promise<EventEnvelope<TEvent> | undefined> {
    return this.enqueue(runId, async () => {
      ...
      if (encoded.event.type !== type) {
        const actualType =
          typeof encoded.event.type === 'string' ? encoded.event.type : 'a non-string event type';
        throw new PersistenceCorruptionError({
          code: 'integrity',
          storage: this.filePath(runId),
          line: pointer.sequence + 2,
          byteOffset: pointer.byteOffset,
          message: `Event-type index for ${type} resolves to ${actualType}.`
        });
      }
  ...
  async verifyIntegrity(runId: string): Promise<LedgerIntegrityReport> {
    ...
    if (errors.length > 0) {
      await this.quarantine(
        runId,
        corruption ??
          new PersistenceCorruptionError({
            code: 'integrity',
            storage: this.filePath(runId),
            line: 1,
            byteOffset: 0,
    ...
    try {
      ...
      try {
        await handle.writeFile(
          `${JSON.stringify({ type: 'event_stream', version: 1, runId, createdAt: new Date().toISOString() } satisfies EventStreamHeader)}\n`,
          'utf8'
        );
        await handle.sync();
      } finally {
    ...
    if (
      ...
      if (index.completeBytes > committedBytes) {
        throw new PersistenceCorruptionError({
          code: 'integrity',
          storage: filePath,
          line: index.recordCount + 2,
          byteOffset: committedBytes,
          message: 'Event stream was truncated before its indexed tail.'
        });
      }
      index = undefined;
    }
  ...
  private async rebuildIndex(
    ...
    for await (const line of readJsonlLines(filePath, { endOffset: completeBytes })) {
      if (first) {
        ...
        if (!isEventHeader(header, runId))
          throw corruption('invalid_header', filePath, line, 'Event stream header is invalid.');
    ...
    if (first)
      throw new PersistenceCorruptionError({
        ...
        line: 1,
        byteOffset: 0,
        message: 'Event stream header is missing.'
      });
  ...
  private async acceptIndexedAppend(
    runId: string,
    index: EventAppendIndex,
  ...
  private async persistedTailMatches(runId: string, index: EventAppendIndex): Promise<boolean> {
    ...
    try {
      ...
      return (
        found?.sequence === index.tail.sequence &&
  ...
  private async writeTailIndex(runId: string, index: EventAppendIndex): Promise<void> {
    const content = {
      ...
      latestByType: Object.fromEntries(
        [...index.latestByType.entries()].sort(([left], [right]) =>
  ...
  private async idempotencyEntry(runId: string, key: string): Promise<IdempotencyIndexEntry | undefined> {
    try {
      ...
      try {
        ...
        throw new PersistenceCorruptionError({
          code: 'integrity',
          storage: this.idempotencyPath(runId, key),
          line: 1,
          byteOffset: 0,
          message: `Idempotency index could not be rebuilt: ${errorMessage(rebuildError)}`
        });
      }
    }
  }
    ...
    if (existing !== undefined) {
      if (existing.eventDigest !== entry.eventDigest || existing.receipt.hash !== entry.receipt.hash) {
        throw new PersistenceCorruptionError({
          ...
          line: 1,
          byteOffset: 0,
          message: 'Idempotency key identifies conflicting ledger records.'
        });
      }
      return;
    }
  ...
  private async classifyFailedConditionalAppend(
    runId: string,
    attempted: EncodedEnvelope,
    cause: unknown
  ): Promise<ConditionalEventAppendResult> {
    this.indexes.delete(runId);
    try {
      const index = await this.refreshIndex(runId, true);
      const matching =
        attempted.idempotencyKey === undefined
          ? undefined
          : await this.idempotencyEntry(runId, attempted.idempotencyKey);
...
function parseEnvelope(value: unknown, runId: string): EncodedEnvelope {
  ...
  if (
    ...
    record.runId !== runId ||
    !nonnegativeInteger(record.sequence) ||
    typeof record.timestamp !== 'string' ||
    record.schemaVersion !== '1' ||
    !isEventActor(record.actor) ||
    !nonnegativeInteger(record.driverGeneration) ||
    typeof record.hash !== 'string' ||
    !jsonObject(record.event)
  )
    throw new Error('Event envelope fields are invalid.');
  if (record.causationId !== undefined && typeof record.causationId !== 'string')
    throw new Error('causationId is invalid.');
  if (record.correlationId !== undefined && typeof record.correlationId !== 'string')
    throw new Error('correlationId is invalid.');
  if (record.previousHash !== undefined && typeof record.previousHash !== 'string')
    throw new Error('previousHash is invalid.');
...
function validateNextEnvelope(
  previous: EventTailRecord | undefined,
...
function parseEnvelopeLine(line: JsonlLine, storage: string, runId: string): EncodedEnvelope {
  ...
  try {
    ...
    throw corruption('invalid_json', storage, line, `Invalid JSON: ${errorMessage(error)}`);
  }
  try {
    return parseEnvelope(raw, runId);
  } catch (error) {
    throw corruption('invalid_record', storage, line, errorMessage(error));
  }
}
...
function parseTailIndex(value: unknown, runId: string): EventAppendIndex {
  ...
  if (
    ...
    !nonnegativeInteger(record.completeBytes) ||
    typeof record.boundaryMarker !== 'string'
  ) {
  ...
  const encodedTypes = parseJsonObject(record.latestByType, {
    maxDepth: 3,
  ...
  for (const [type, encodedPointer] of Object.entries(encodedTypes)) {
    ...
    if (pointer.sequence >= record.recordCount)
      throw new TypeError(`Event-type index ${type} points beyond the ledger tail.`);
...
function parseEventTailRecord(value: unknown, label: string): EventTailRecord {
  ...
  if (
    ...
    !isEventActor(candidate.actor) ||
    typeof candidate.hash !== 'string' ||
    !nonnegativeInteger(candidate.driverGeneration) ||
    !nonnegativeInteger(candidate.byteOffset)
  ) {
...
function parseIdempotencyEntry(value: unknown, runId: string, key: string): IdempotencyIndexEntry {
  ...
  if (
    ...
    !jsonObject(record.receipt)
  ) {
    throw new TypeError('Idempotency index fields are invalid.');
  }
  const receiptRecord = record.receipt;
  if (
    typeof receiptRecord.eventId !== 'string' ||
    receiptRecord.runId !== runId ||
    !nonnegativeInteger(receiptRecord.sequence) ||
    typeof receiptRecord.timestamp !== 'string' ||
    receiptRecord.schemaVersion !== '1' ||
    !isEventActor(receiptRecord.actor) ||
    typeof receiptRecord.hash !== 'string' ||
    !nonnegativeInteger(receiptRecord.driverGeneration)
  ) {
    throw new TypeError('Idempotency receipt fields are invalid.');
  }
  for (const field of ['causationId', 'correlationId', 'idempotencyKey', 'previousHash'] as const) {
    if (receiptRecord[field] !== undefined && typeof receiptRecord[field] !== 'string')
      throw new TypeError(`Idempotency receipt ${field} is invalid.`);
  }
...
function domainEnvelope<TEvent extends TypedEvent>(
  ...
  return Object.freeze({
    ...
    actor: encoded.actor,
    ...(encoded.causationId === undefined ? {} : { causationId: encoded.causationId }),
    ...(encoded.correlationId === undefined ? {} : { correlationId: encoded.correlationId }),
    ...(encoded.idempotencyKey === undefined ? {} : { idempotencyKey: encoded.idempotencyKey }),
    ...(encoded.previousHash === undefined ? {} : { previousHash: encoded.previousHash }),
    hash: encoded.hash,
...
function parseLine(line: JsonlLine, storage: string): unknown {
  try {
    ...
    throw corruption('invalid_json', storage, line, errorMessage(error));
  }
}
function corruption(
...
export async function atomicWritePrivateJson(filePath: string, value: JsonObject): Promise<void> {
...
export async function withPersistenceFileLock<T>(
  filePath: string,
  timeoutMs: number,
  staleMs: number,
  ...
  for (;;) {
    try {
      ...
      if (Date.now() - started >= timeoutMs)
        throw new Error(`Timed out acquiring persistence lock: ${lockPath}`, { cause: error });
      await delay(10);
    }
  }
  const assertOwned = async (): Promise<void> => {
    ...
    try {
      ...
      throw new PersistenceConflictError('Persistence lock ownership was lost.');
    }
    if (!isRecord(owner) || owner.token !== token || owner.pid !== process.pid)
      throw new PersistenceConflictError('Persistence lock ownership was lost.');
  };
  try {
