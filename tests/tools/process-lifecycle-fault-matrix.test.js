...
test('process lifecycle fault matrix preserves one settled terminal truth', async (t) => {
  await t.test('exit before supervisor reachability leaves no owned process', async () => {
    ...
    await assert.rejects(startCommand(manager, request(missing, 'process.exit(0)')));
    assert.equal(manager.activeCount(owner.runId), 0);
    assert.deepEqual(await manager.disposeOwner(owner.runId), []);
  });

  await t.test(
    'supervisor death stops its authenticated process tree before terminal publication',
    async () => {
      const delivered = [];
      ...
      while (result.status === 'running')
        result = await manager.query(result.processId, 1_000, 50, result.cursorEnd, owner);
      const heartbeatAtTerminal = await readFile(heartbeat, 'utf8');
      await new Promise((resolve) => setTimeout(resolve, 100));
      assert.equal(
        await readFile(heartbeat, 'utf8'),
        heartbeatAtTerminal,
        'descendant work stopped before terminal publication'
      );
      ...
      const firstCleanup = await manager.disposeOwner(owner.runId);
      const secondCleanup = await manager.disposeOwner(owner.runId);
  ...
  for (const race of [
    ...
    await t.test(race.name + ' produce one stable outcome', async () => {
      ...
      let result = await startCommand(
        manager,
        request('.', 'setTimeout(() => process.exit(0), 35)', { timeoutMs: race.timeoutMs })
      );
      if (race.stop) {
        const outcomes = await Promise.all([
          manager.terminate(result.processId, owner),
          manager.terminate(result.processId, owner)
        ]);
        ...
        while (result.status === 'running')
          result = await manager.query(result.processId, 100, 20, result.cursorEnd, owner);
      }
      ...
      assert.equal(
        ['exited', race.stop ? 'stopped' : 'timed_out'].includes(result.status),
        true,
        result.status
      );
      await manager.acknowledgeTerminalReport(result.processId);
  ...
  await t.test('shutdown drains output and observer failures cannot change truth', async () => {
    ...
    const { manager, root } = await context();
    const script =
      "let n=0;const timer=setInterval(()=>{process.stdout.write(String(n++)+'\\n');if(n===8){clearInterval(timer);process.exit(0)}},5)";
    let result = await startCommand(
      manager,
      request('.', script, {
    ...
    while (result.status === 'running')
      result = await manager.query(result.processId, 1_000, 20, result.cursorEnd, owner);
...
test('terminal-state recovery rejects missing, malformed, stale, unauthenticated, misowned, and invalid states', async (t) => {
  ...
  const [validReport] = await validManager.disposeOwner('recovered-run');
  assert.equal(validReport.result.processId, valid.processId);
  assert.deepEqual(validReport.result.owner, {
  ...
  const cases = [
    ...
    [
      'stale identity and process',
      async (fixture) => {
        await writeFile(fixture.stateFile, staleState);
      }
    ],
    [
      'incorrect authentication',
      mutateState((state) => {
        state.proof = '0'.repeat(64);
      })
    ],
    [
      'incorrect execution ownership',
      mutateState((state) => {
        state.owner.runId = 'other-run';
      })
    ],
    [
  ...
  for (const [name, mutate] of cases) {
    await t.test(name, async () => {
      ...
      assert.equal(
        reconciliation.unresolved.some((item) => item.processId === fixture.processId),
        true
      );
...
async function readLedger(directory, processId) {
  return JSON.parse(await readFile(path.join(directory, `${processId}.json`), 'utf8'));
}
...
function recoveredManager(fixture) {
  return new LocalCommandExecution({
    artifactRepository: new LocalArtifactRepository({
      rootDir: path.join(fixture.root, 'recovered-artifacts')
    }),
    rootedFileAuthority: testRootedFileAuthority(fixture.root),
