import { commandExecutionResources } from '@agent-core/tools';
...
const secrets = [
  ...
  'password-value-789',
  'environment-value-456',
  'process-value-123'
];
const sessionBinding = Object.freeze({
  schemaId: 'agent-core.tests/durable-redaction',
  schemaVersion: 1,
  subject: Object.freeze({ application: 'durable-redaction' })
});
const profile = {
  id: 'scripted',
  provider: 'scripted',
...
const secretResult = defineTool({
  name: 'secret_result',
  implementationId: 'tests/secret-result@1',
  description: 'secret',
  ...
  invoke: async () => ({
    ...
    metadata: { apiKey: secrets[0], nested: { password: secrets[2] } }
  })
});
const secretFailure = defineTool({
  name: 'secret_failure',
  implementationId: 'tests/secret-failure@1',
...
test('durable event and session JSONL redact tool, metadata, failure, and process secrets', async () => {
  ...
  const agent = new AgentRuntime({ maxOutputTokens: 64,
    ...
    tools: [secretResult, secretFailure, execCommandTool],
    resources: commandExecutionResources(manager),
