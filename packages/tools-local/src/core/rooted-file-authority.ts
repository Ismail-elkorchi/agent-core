...
export class RootedFileAuthority {
  ...
  private constructor(
    ...
    deniedRootEntries: ReadonlySet<string>
  ) {
  ...
  static adopt(rootPath: string, options: RootedFileAuthorityOptions = {}): RootedFileAuthority {
    if (process.platform !== 'linux') {
      throw new Error(
        `Root-bound host file access is unavailable on ${process.platform}; this platform lacks the required package-owned handle-relative resolver.`
      );
    }
    if (typeof rootPath !== 'string' || rootPath.trim().length === 0)
      throw new TypeError('Root directory must be a non-empty path.');
    ...
    try {
      ...
      if (openedPath.endsWith(' (deleted)'))
        throw new Error(`Root directory was removed during adoption: ${displayPath}`);
      const denied = ownDeniedEntries([
        ...DEFAULT_DENIED_ROOT_ENTRIES,
  ...
  async openFile(requestedPath: string): Promise<RootedFileHandle> {
    ...
    if (rootedDirectory === '.')
      throw new ToolInputError('Root directory is not a regular file.', { path: requestedPath });
    ...
    try {
      ...
      try {
        ...
        return makeRootedFileHandle(rootedDirectory, handle, stat, () => {
          this.#assertOpen();
        });
      } catch (error) {
        await handle.close();
      ...
      if (nodeCode(error) === 'ELOOP')
        throw new ToolInputError(`Refusing a symbolic-link file path: ${requestedPath}`, {
          path: requestedPath
        });
      throw error;
    } finally {
  ...
  async openDirectory(requestedPath: string): Promise<RootedDirectoryHandle> {
    const rootedDirectory = this.canonicalPath(requestedPath);
    const opened = await this.#openDirectorySegments(
    ...
    const authority: RootedDirectoryHandle = Object.freeze({
      ...
      async entries() {
        ...
        const dirents = await import('node:fs/promises').then(({ readdir }) =>
          readdir(fdPath(opened.handle.fd), { withFileTypes: true })
        );
        return Object.freeze(
          dirents.map((entry) =>
            Object.freeze({
  ...
  async [openMutationDirectory](requestedPath: string): Promise<RootedMutationDirectory> {
    ...
    const authority: RootedMutationDirectory = Object.freeze({
      ...
      async status(name: string) {
        ...
        let stat: BigIntStats;
        try {
          stat = await lstat(entryPath(leaf), { bigint: true });
        } catch (error) {
          if (nodeCode(error) === 'ENOENT')
            return Object.freeze({ kind: 'absent', path: joinPath(rootedDirectory, leaf) });
          throw error;
        }
      ...
      async createDirectory(name: string, mode: number) {
        assertOpen();
        await mkdir(entryPath(name), { mode: mode & 0o777 });
      },
      async rename(sourceName: string, destinationName: string) {
        assertOpen();
        await rename(entryPath(sourceName), entryPath(destinationName));
      },
      async link(sourceName: string, destinationName: string) {
        assertOpen();
        await link(entryPath(sourceName), entryPath(destinationName));
      },
      async removeFile(name: string) {
        assertOpen();
        try {
          await unlink(entryPath(name));
        } catch (error) {
  ...
  async #openDirectorySegments(
    ...
    requestedPath: string
  ): Promise<{ readonly handle: FileHandle; close(): Promise<void> }> {
    this.#assertOpen();
    ...
    try {
      ...
      for (const segment of segments) {
        ...
        try {
          next = await open(
            `${fdPath(current.fd)}/${segment}`,
            fsConstants.O_RDONLY | fsConstants.O_DIRECTORY | fsConstants.O_NOFOLLOW
          );
        } catch (error) {
          if (nodeCode(error) === 'ELOOP' || nodeCode(error) === 'ENOTDIR') {
            throw new ToolInputError(
              `Refusing an aliased or non-directory path component: ${requestedPath}`,
              { path: requestedPath, segment }
            );
          }
          throw error;
        }
        try {
          this.#assertMount(next.fd, requestedPath);
        } catch (error) {
          await next.close();
          throw error;
        }
  ...
  #assertMount(fd: number, requestedPath: string): void {
    ...
    if (mountId !== this.#rootMountId) {
      throw new ToolInputError(`Path crosses the adopted root mount: ${requestedPath}`, {
        path: requestedPath
      });
    }
  }
...
export function openRootedMutationDirectory(
  root: RootedFileAuthority,
  requestedPath: string
): Promise<RootedMutationDirectory> {
  if (!isRootedFileAuthority(root))
    throw new TypeError('Rooted mutations require an adopted RootedFileAuthority.');
  ...
  return Object.freeze({
    ...
    async readAll(maxBytes: number) {
      ...
      if (!Number.isSafeInteger(maxBytes) || maxBytes < 0)
        throw new TypeError('Maximum read bytes must be a non-negative safe integer.');
      const current: BigIntStats = await handle.stat({ bigint: true });
      if (current.size > BigInt(maxBytes))
        throw new ToolInputError(`File exceeds the read limit: ${pathValue}`, {
          path: pathValue,
          bytes: String(current.size),
          maxBytes
        });
      const chunks: Buffer[] = [];
      let offset = 0;
      for (;;) {
        const chunk = Buffer.allocUnsafe(Math.min(64 * 1024, maxBytes - offset + 1));
...
function normalizeRootedPath(
  requestedPath: string,
  deniedRootEntries: ReadonlySet<string>,
  allowReservedInternal = false
): string {
  if (typeof requestedPath !== 'string' || requestedPath.trim().length === 0)
    throw new ToolInputError('Path cannot be empty.', { path: requestedPath });
  ...
  if (
    ...
    throw new ToolInputError(`Absolute, UNC, and device paths are not allowed: ${requestedPath}`, {
      path: requestedPath
    });
  }
  const segments = requestedPath.split('/').filter((segment) => segment !== '' && segment !== '.');
