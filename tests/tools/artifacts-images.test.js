...
test('read_artifact returns ranges, continuation offsets, and typed text content', async () => {
  ...
  const artifact = await repository.store({
    label: 'sample',
    content: new TextEncoder().encode('0123456789'),
    mediaType: 'text/plain; charset=utf-8'
  });
  const observation = await invokeToolCall(
    jsonToolCall('read_artifact', { artifactId: artifact.artifactId, offset: 2, byteCount: 4 }),
    tools,
    {
      policy: { allowedRisks: ['read'] },
      services: { artifactRepository: repository }
    }
  );
  ...
  assert.deepEqual(observation.content, [
    { type: 'text', text: '2345', mediaType: 'text/plain; charset=utf-8' }
  ]);
});
...
test('view_image stores image bytes and returns an image content reference without a data URL', async () => {
  ...
  const observation = await invokeToolCall(
    jsonToolCall('view_image', { path: 'image.png', detail: 'original' }),
    tools,
    {
      policy: { allowedRisks: ['read'] },
      services: {
...
test('view_image rejects replacement, growth, truncation, invalid headers, and excessive pixels', async () => {
  for (const mutation of ['replacement', 'growth', 'truncation']) {
    ...
    if (mutation === 'replacement')
      await writeFile(path.join(root, 'replacement.png'), pngBytes(4, 5));
    let changed = false;
    const result = await invokeToolCall(jsonToolCall('view_image', { path: 'image.png' }), tools, {
      ...
      async emitProgress(progress) {
        ...
        if (mutation === 'replacement') {
          try {
            await rename(path.join(root, 'replacement.png'), target);
          } catch (error) {
  ...
  await writeFile(
    path.join(invalidRoot, 'bad.png'),
    Buffer.from([137, 80, 78, 71, 13, 10, 26, 10])
  );
  const invalid = await invokeToolCall(jsonToolCall('view_image', { path: 'bad.png' }), tools, {
    policy: { allowedRisks: ['read'] },
    services: {
      rootedFileAuthority: testRootedFileAuthority(invalidRoot),
...
test('artifact repositories verify small ranges without breaking UTF-8 boundaries', async () => {
  ...
  for (const repository of [
    ...
    new LocalArtifactRepository({ rootDir: root })
  ]) {
...
test('public and protected artifacts have distinct lookup and filesystem visibility', async () => {
  ...
  const publicRef = await repository.store({
    label: 'public',
    content: new TextEncoder().encode('public'),
    mediaType: 'text/plain'
  });
  const protectedRef = await repository.storeProtected({
    label: 'raw',
    content: new TextEncoder().encode('secret raw'),
    mediaType: 'text/plain'
  });
  ...
  if (process.platform !== 'win32') {
    ...
    assert.equal(
      (await stat(path.join(root, 'protected', protectedRef.artifactId))).mode & 0o777,
      0o600
    );
  }
  const denied = await invokeToolCall(
    jsonToolCall('read_artifact', { artifactId: protectedRef.artifactId }),
    tools,
    {
      policy: { allowedRisks: ['read'] },
      services: { artifactRepository: repository }
...
test('verified range cache reuses hashes only for unchanged file identity', async () => {
  ...
  const repository = new LocalArtifactRepository({
    rootDir: root,
    onVerificationScan: (artifactId) => scans.push(artifactId)
  });
  ...
  await assert.rejects(
    repository.readVerifiedRange(artifact, { offset: 0, length: 4 }),
    /SHA-256/u
  );
