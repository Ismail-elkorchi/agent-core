...
import {
  commitTextFilePatchTransaction,
  recoverTextFilePatchTransactions
} from '@agent-core/tools-local/testing/text-write';
...
test('apply_patch parses once into one canonical tree and applies add, update, move, and delete transactionally', async () => {
  ...
  const observation = await invokeToolCall(
    textToolCall('apply_patch', document),
    [applyPatchTool],
    {
      policy,
  ...
  assert.deepEqual([...observation.output.changedPaths].sort(), [
    'added.txt',
    'delete.txt',
    'move.txt',
    'moved.txt',
...
test('tool policy denies destructive patches without destructive authority', async () => {
  ...
  await writeFile(path.join(root, 'delete.txt'), 'delete\n');
  const observation = await invokeToolCall(
    textToolCall('apply_patch', patch(['*** Delete File: delete.txt'])),
    [applyPatchTool],
    {
  ...
  assert.match(JSON.stringify(observation.output.details), /delete|destructive/iu);
...
test('apply_patch enforces raw SHA-256 preconditions and keeps dry runs non-mutating', async () => {
  ...
  const wrong = await invokeToolCall(
    {
      name: 'apply_patch',
      input: {
        kind: 'json',
        value: { patch: document, expectedOldSha256: { 'note.txt': '0'.repeat(64) } }
      }
    },
    [applyPatchTool],
    {
  ...
  const dryRun = await invokeToolCall(
    {
      ...
      input: {
        kind: 'json',
        value: { patch: document, dryRun: true, expectedOldSha256: { 'note.txt': expected } }
      }
    },
    [applyPatchTool],
    {
  ...
  assert.equal(dryRun.kind, 'result');
...
test('apply_patch preserves all four transaction outcome semantics', async () => {
  const outcomes = [
    { outcome: 'committed', cleanup: { status: 'succeeded', diagnostics: [], strandedPaths: [] } },
    {
      ...
      cleanup: {
        ...
        diagnostics: [
          {
            operation: 'remove_patch_journal',
            path: '.agent-core/transactions/patch/tx',
            message: 'busy'
          }
        ],
        strandedPaths: ['.agent-core/transactions/patch/tx']
      }
    },
    {
      outcome: 'rolled_back',
      failure: { operation: 'commit_write', path: 'note.txt', message: 'write failed' },
      rollback: { status: 'succeeded', diagnostics: [], strandedPaths: [] }
    },
    {
      ...
      rollback: {
        status: 'uncertain',
        diagnostics: [{ operation: 'restore_backup', path: 'note.txt', message: 'restore failed' }],
        strandedPaths: ['note.txt']
      }
    }
  ];
  for (const transaction of outcomes) {
    ...
    const planning = await planToolCall(
      textToolCall('apply_patch', patch(['*** Update File: note.txt', '@@', '-old', '+new'])),
      [applyPatchTool],
      context
    );
    assert.equal(planning.ok, true);
    const observation = await applyPatchWithAuthority(planning.plan.canonicalSnapshot, context, {
      async commit() {
        return transaction;
      }
    });
    ...
    assert.equal(
      observation.output.applicationStatus,
    ...
    if (transaction.outcome === 'committed' || transaction.outcome === 'committed_with_residue') {
      ...
      assert.equal(
        ...
        transaction.outcome === 'rolled_back' ? 'settled' : 'unknown'
      );
      assert.deepEqual(observation.output.changedPaths, []);
      assert.equal(
        observation.output.files[0].finalState,
        transaction.outcome === 'rolled_back' ? 'unchanged' : 'uncertain'
      );
    }
...
test('apply_patch reports no-op without a transaction outcome', async () => {
  ...
  const result = await invokeToolCall(
    textToolCall('apply_patch', patch(['*** Update File: note.txt', '@@', '-same', '+same'])),
    [applyPatchTool],
    {
      policy,
      services: { rootedFileAuthority: testRootedFileAuthority(root) }
...
test('apply_patch dynamically requires a transaction directory only for writes', async () => {
  ...
  const document = patch(['*** Update File: note.txt', '@@', '-old', '+new']);
  const dryPlan = await planToolCall(
    createToolCall({
      name: 'apply_patch',
      input: { kind: 'json', value: { patch: document, dryRun: true } }
    }),
    [applyPatchTool],
    base
  );
  ...
  const writePlan = await planToolCall(
    textToolCall('apply_patch', document),
    [applyPatchTool],
    base
  );
  ...
  const withDirectory = {
    ...
    services: {
      ...base.services,
      patchJournal: testPatchJournal(base.services.rootedFileAuthority)
    }
  };
  const appliedPlan = await planToolCall(
    textToolCall('apply_patch', document),
    [applyPatchTool],
    base
  );
...
test('apply_patch dry runs require only read access and do not create transaction state', async () => {
  ...
  const document = patch(['*** Update File: note.txt', '@@', '-old', '+new']);
  const observation = await invokeToolCall(
    { name: 'apply_patch', input: { kind: 'json', value: { patch: document, dryRun: true } } },
    [applyPatchTool],
    {
      policy: { allowedRisks: ['read'] },
      services: { rootedFileAuthority: testRootedFileAuthority(root) }
...
test('apply_patch reports parser and matching diagnostics without mutating files', async () => {
  ...
  const malformed = await invokeToolCall(
    textToolCall('apply_patch', 'not a patch'),
    [applyPatchTool],
    {
      policy,
      services: { rootedFileAuthority: testRootedFileAuthority(root) }
  ...
  const contextOnly = await invokeToolCall(
    textToolCall('apply_patch', patch(['*** Update File: note.txt', '@@', ' section one'])),
    [applyPatchTool],
  ...
  const ambiguous = await invokeToolCall(
    textToolCall('apply_patch', patch(['*** Update File: note.txt', '@@', '-value', '+VALUE'])),
    [applyPatchTool],
    { policy, services: { rootedFileAuthority: testRootedFileAuthority(root) } }
  );
  ...
  assert.equal(narrowed.kind, 'result');
...
test('apply_patch rejects invalid targets atomically under host limits', async () => {
  ...
  const localToolConfiguration = {
    ...DEFAULT_LOCAL_TOOL_CONFIGURATION,
    applyPatch: {
      ...DEFAULT_LOCAL_TOOL_CONFIGURATION.applyPatch,
      maxFileBytes: 128,
      maxNewBytesPerFile: 128
    }
  };
  const document = patch([
  ...
  assert.equal(observation.kind, 'failure');
  assert.deepEqual(observation.output.details.failures.map((failure) => failure.reason).sort(), [
    'already_exists',
    'binary',
    'not_file',
    'not_found',
    ...
    writes: [
      {
        ...
        mode,
        overwrite: true,
        expectedCurrentSha256: createHash('sha256').update('old first\n').digest('hex'),
        expectedCurrentIdentity: firstIdentity
      }
    ],
    removes: []
  });
...
test('patch recovery preserves directories and files that its journal cannot prove it owns', async () => {
  ...
  await writeJournalEnvelope(path.join(unprovenDirectory, 'transaction.json'), {
    version: 1,
    transactionId: unprovenId,
    transactionDigest: '1'.repeat(64),
    phase: 'planned',
    createdDirectories: [{ path: 'external-empty' }],
    writes: [],
    removes: []
  });
  const journal = TextPatchJournal.adopt(journalPath);
  await assert.rejects(
    recoverTextFilePatchTransactions(root, journal),
    /cannot prove ownership|uncertain/iu
  );
  ...
  const corruptJournalPath = await mkdtemp(
    path.join(tmpdir(), 'agent-core-patch-corrupt-journal-')
  );
  ...
  await writeJournalEnvelope(path.join(corruptDirectory, 'transaction.json'), {
    ...
    writes: [
      {
        path: 'target.txt',
        stageName: 'victim.txt',
        newSha256: createHash('sha256').update('x').digest('hex'),
        mode: 0o600,
        overwrite: false,
        expectedAbsent: true
      }
  ...
  await assert.rejects(
    recoverTextFilePatchTransactions(root, corruptJournal),
    /invalid patch stage/iu
  );
  assert.equal(await readFile(path.join(rootPath, 'victim.txt'), 'utf8'), 'keep\n');
  corruptJournal.close();

  const checksumJournalPath = await mkdtemp(
    path.join(tmpdir(), 'agent-core-patch-checksum-journal-')
  );
  ...
  await writeFile(
    path.join(checksumDirectory, 'transaction.json'),
    JSON.stringify({ version: 1, payload: checksumManifest, sha256: '0'.repeat(64) }),
    { mode: 0o600 }
  );
  const checksumJournal = TextPatchJournal.adopt(checksumJournalPath);
  await assert.rejects(
    recoverTextFilePatchTransactions(root, checksumJournal),
    /checksum failed/iu
  );
...
test('patch journal operation authority expires when its callback returns', async () => {
  ...
  let retained;
  await journal.withAuthority(root, async (authority) => {
    retained = authority;
  });
...
test(
  'a hostile parent swap cannot redirect a structured mutation outside the adopted root',
      ...
      for (let attempt = 0; attempt < 25; attempt += 1) {
        try {
          await invokeToolCall(
            textToolCall(
              'apply_patch',
              patch([`*** Add File: branch/new-${String(attempt)}.txt`, '+inside'])
            ),
            [applyPatchTool],
            {
              policy,
          ...
          /* A concurrent alias or missing parent is an admitted conflict. */
        }
      }
...
test('patch commit revalidates every hash and absent destination before mutation', async () => {
    ...
    writes: [
      {
        ...
        content: 'patched\n',
        overwrite: true,
        expectedCurrentSha256: initialSha,
        expectedCurrentIdentity: initialIdentity
      }
    ],
    removes: []
  });
  ...
  const appeared = await commitTextFilePatchTransaction(rootedFileAuthority, journal, {
    writes: [
      { path: 'destination.txt', content: 'patched\n', overwrite: false, expectedAbsent: true }
    ],
    removes: []
  });
...
test('one journal lock serializes same-process patches on different and identical files', async () => {
    ...
    writes: [
      {
        ...
        expectedCurrentIdentity: await rootedFileAuthority.fileIdentity(file)
      }
    ],
    removes: []
  });
  const distinct = await Promise.all([
    transaction('a.txt', 'a1\n', shaA).then((value) =>
      commitTextFilePatchTransaction(rootedFileAuthority, journal, value)
    ),
  ...
  assert.deepEqual(
    distinct.map((item) => item.outcome),
    ['committed', 'committed']
  );
  ...
  return new Promise((resolve, reject) => {
    ...
    child.once('close', (code) =>
      code === 0
        ...
        : reject(new Error(stderr || `child exited ${String(code)}`))
    );
  });
}
