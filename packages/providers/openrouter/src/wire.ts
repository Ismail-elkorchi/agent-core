export interface OpenRouterModelCatalog {
  readonly data?: readonly OpenRouterModelRecord[];
}
...
export interface OpenRouterWireToolCall {
  ...
  readonly function?: Readonly<Record<string, unknown>> & {
    readonly name?: string;
...
export interface OpenRouterUsage {
  ...
  readonly prompt_tokens_details?: Readonly<Record<string, unknown>> & {
    readonly cached_tokens?: number;
    readonly cache_write_tokens?: number;
  };
  readonly completion_tokens_details?: Readonly<Record<string, unknown>> & {
...
export function decodeOpenRouterModelCatalog(value: unknown): OpenRouterModelCatalog {
  ...
  return Object.freeze({
    ...(record.data === undefined
      ? {}
      : { data: array(record.data, 'OpenRouter model catalog.data', decodeModelRecord) })
  });
}
...
export function decodeOpenRouterChatResponse(value: unknown): OpenRouterChatResponse {
  ...
  const error =
    record.error === undefined ? undefined : decodeError(record.error, 'OpenRouter chat response.error');
  return Object.freeze({
    ...stringField(record, 'id', 'OpenRouter chat response'),
    ...stringField(record, 'model', 'OpenRouter chat response'),
    ...(record.choices === undefined
      ? {}
      : { choices: array(record.choices, 'OpenRouter chat response.choices', decodeChoice) }),
    ...(record.usage === undefined
      ? {}
      : { usage: decodeUsage(record.usage, 'OpenRouter chat response.usage') }),
...
function decodeModelRecord(value: unknown, label: string): OpenRouterModelRecord {
  ...
  const architecture =
    record.architecture === undefined
      ? undefined
      : decodeArchitecture(record.architecture, `${label}.architecture`);
  const pricing =
    record.pricing === undefined ? undefined : decodePricing(record.pricing, `${label}.pricing`);
  const topProvider =
    record.top_provider === null
      ? null
      : record.top_provider === undefined
        ? undefined
        : decodeTopProvider(record.top_provider, `${label}.top_provider`);
  const reasoning =
    record.reasoning === undefined ? undefined : decodeReasoning(record.reasoning, `${label}.reasoning`);
  return Object.freeze({
    ...
    ...(record.supported_parameters === undefined
      ? {}
      : { supported_parameters: stringArray(record.supported_parameters, `${label}.supported_parameters`) }),
...
function decodeChoice(value: unknown, label: string): OpenRouterChoice {
  ...
  if (finishReason !== undefined && finishReason !== null && typeof finishReason !== 'string')
    throw new Error(`${label}.finish_reason must be a string or null.`);
  return Object.freeze({
...
function decodeMessage(value: unknown, label: string): OpenRouterResponseMessage {
  ...
  if (content !== undefined && content !== null && typeof content !== 'string' && !Array.isArray(content))
    throw new Error(`${label}.content must be text, an array, or null.`);
  const contentParts = Array.isArray(content)
    ? Object.freeze(content.map((part, index) => object(part, `${label}.content[${String(index)}]`)))
    : content;
  ...
  return Object.freeze({
    ...
    ...(record.reasoning_content === undefined
      ? {}
      : { reasoning_content: nullableString(record.reasoning_content) }),
    ...(record.reasoning_details === undefined ? {} : { reasoning_details: record.reasoning_details }),
    ...(record.tool_calls === undefined
      ? {}
      : { tool_calls: array(record.tool_calls, `${label}.tool_calls`, decodeToolCall) })
  });
}
...
function decodeToolCall(value: unknown, label: string): OpenRouterWireToolCall {
  ...
  if (fn) {
    ...
    if (argumentsValue !== undefined && typeof argumentsValue !== 'string' && !isObject(argumentsValue))
      throw new Error(`${label}.function.arguments must be a string or object.`);
    decodedFunction = Object.freeze({
      ...stringField(fn, 'name', `${label}.function`),
  ...
  return Object.freeze({
    ...
    ...(decodedFunction === undefined ? {} : { function: decodedFunction })
  });
}
...
function decodeUsage(value: unknown, label: string): OpenRouterUsage {
  ...
  return Object.freeze({
    ...
    ...(record.prompt_tokens_details === undefined
      ? {}
      : {
          prompt_tokens_details: decodeDetails(
            record.prompt_tokens_details,
            `${label}.prompt_tokens_details`,
            ['cached_tokens', 'cache_write_tokens']
          )
        }),
    ...(record.completion_tokens_details === undefined
      ? {}
      : {
          completion_tokens_details: decodeDetails(
            record.completion_tokens_details,
            `${label}.completion_tokens_details`,
            ['reasoning_tokens']
          )
        })
  });
...
function decodeArchitecture(value: unknown, label: string): OpenRouterModelRecord['architecture'] {
  const record = object(value, label);
  return Object.freeze({
    ...(record.input_modalities === undefined
      ? {}
      : { input_modalities: stringArray(record.input_modalities, `${label}.input_modalities`) }),
    ...(record.output_modalities === undefined
      ? {}
      : { output_modalities: stringArray(record.output_modalities, `${label}.output_modalities`) })
  });
}
...
function decodeTopProvider(
  value: unknown,
  label: string
): Exclude<OpenRouterModelRecord['top_provider'], null | undefined> {
  const record = object(value, label);
  return Object.freeze({
    ...nullableNumberField(record, 'context_length', label),
    ...nullableNumberField(record, 'max_completion_tokens', label)
  });
}
...
function decodeReasoning(value: unknown, label: string): NonNullable<OpenRouterModelRecord['reasoning']> {
  ...
  if (efforts !== undefined && efforts !== null && !Array.isArray(efforts))
    throw new Error(`${label}.supported_efforts must be an array or null.`);
  return Object.freeze({
    ...(efforts === undefined
      ? {}
      : { supported_efforts: efforts === null ? null : stringArray(efforts, `${label}.supported_efforts`) }),
...
function decodeError(value: unknown, label: string): NonNullable<OpenRouterChatResponse['error']> {
  ...
  if (record.code !== undefined && typeof record.code !== 'string' && typeof record.code !== 'number')
    throw new Error(`${label}.code must be a string or number.`);
  return Object.freeze({
    ...(record.code === undefined ? {} : { code: record.code }),
    ...stringField(record, 'message', label),
...
function array<T>(value: unknown, label: string, decode: (value: unknown, label: string) => T): readonly T[] {
...
function integerField(
  ...
  const value = integer(record[key], `${label}.${key}`);
...
function nullableString(value: unknown): string | null {
  ...
  throw new Error('reasoning_content must be a string or null.');
}
