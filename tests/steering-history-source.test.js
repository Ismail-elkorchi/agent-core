...
import { HistoryReader, InMemorySessionRepository, agentEventCodec, sourceRef } from '@agent-core/runtime';
...
test('accepted steering remains an original history source when its session copy is absent', async () => {
  ...
  const accepted = await events.append('original-run', {
    type: 'input.steering.accepted', deliveryId: 'correction', content: 'The corrected identifier is ORCHID-492.'
  });
  await events.append('original-run', { type: 'input.steering.local_applied', deliveryId: 'correction' });
  ...
  assert.equal((await reopened.view()).entries.filter((entry) => entry.type === 'steering').length, 1);
});
