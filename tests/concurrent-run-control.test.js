...
function acceptance(runId) {
  return {
    runId, finalizationId: `${runId}:final`,
    input: { task: 'Exercise independently fenced work.', instructions: [], contextItems: [] },
    configuration: {
      providerId: 'scripted', providerImplementationId: 'scripted@1', model: 'scripted',
      runtimeImplementationId: 'runtime@1', toolImplementationIds: ['read@1'], checks: [],
      disposition: { implementationId: 'accept@1', policyIdentity: {}, policyHash: hashJson({}) }, policyHash: 'policy'
    }
  };
}
...
function effect(runId, id, stage = 'started', generation = 1) {
  const issued = issueEffectStartTicket({
    intent: { effectId: id, ownerId: runId, implementationId: 'read@1', parametersDigest: hashJson(id),
      recovery: { kind: 'unknown' }, exposure: { quantities: [] } },
    ticketId: `${id}:start`, settlementPermitId: `${id}:settle`, driverGeneration: generation,
...
function batch(runId, id, { stage = 'effect_pending', scope = id, dependencies = [], count = 1, maxConcurrency = 3 } = {}) {
  const calls = Array.from({ length: count }, (_, index) => createToolCall({
    id: `${id}:call:${index}`, name: 'read', input: { kind: 'json', value: { index } }
  }));
  const entries = [{ name: 'read', implementationId: 'read@1', definitionHash: hashJson('read') }];
  return {
    kind: 'tools', identity: { turnIndex: id === 'r1' ? 1 : 2, turnId: `${id}:turn`, requestAttempt: 1 },
    toolBatchId: id, calls, maxConcurrency, instructions: [], modelInputModalities: ['text'],
    source: { responseId: id, catalog: { revision: hashJson(entries), entries } },
    modelCalls: calls.map(call => ({ id: call.id, name: call.name, input: call.input, type: 'function', async: true })),
    callStates: calls.map((_, index) => {
      const execution = effect(runId, `${id}:effect:${index}`, stage === 'effect_ready' ? 'ticket_issued' : 'started');
      return {
        stage, toolAttempt: 1, effect: execution,
        plan: {
          toolImplementationId: 'read@1', fingerprint: execution.intent.parametersDigest, canonicalInput: { index },
          effects: { accesses: [{ mode: 'write', scope }], lockScopes: [], recovery: { kind: 'unknown' },
            ...(index && dependencies.length ? { dependsOnCallIndices: dependencies } : {}) },
          binding: { toolImplementationId: 'read@1', authorizationPolicyId: 'policy', executionTargetId: 'memory' },
          authorization: 'allow'
        }
      };
    })
...
function provider(runId) {
  return {
    kind: 'provider', stage: 'effect_pending', identity: { turnIndex: 2, turnId: 'r2:turn', requestAttempt: 1 },
    toolBatchId: 'r2', requestEventId: 'request-r2', responseId: 'r2', effect: effect(runId, 'provider:r2')
  };
}

function observation(id, ok = true) {
  return {
    observationId: `observation:${id}`, createdAt: new Date(0).toISOString(),
    observation: parseToolObservation({ outputSchema: z.strictObject({ value: z.string() }) }, {
      kind: 'result', ok, output: { value: id }, summary: `Completed ${id}`,
...
async function fixture(runId, toolBatches, providerRequests = []) {
  ...
  const state = decodeAgentRunState({ ...initial.state(), revision: 2, phase: { kind: 'active' }, toolBatches, providerRequests });
  await events.appendConditional(runId, { type: 'run.state.changed', state }, {
    idempotencyKey: `${runId}:work`, expectedTail: await events.tail(runId), driverGeneration: 1
  });
...
function settle(runs, runId, work, callIndex = 0, ok = true) {
  ...
  return runs.settleToolEffect(runId, {
    effectId: state.effect.intent.effectId, permit: state.effect.settlementPermit,
...
test('an older driver settles A while R2 independently settles, without replacing either source', async () => {
  ...
  const responseSettlement = settleExternalEffect(later.effect, later.effect.settlementPermit, {
    outcome: 'succeeded', resultDigest: hashJson('r2 response'), exposure: { status: 'known', quantities: [] }
  });
  assert.equal(responseSettlement.status, 'settled');
  await Promise.all([
    initial.settleToolEffect({ effectId: original.callStates[0].effect.intent.effectId,
      permit: original.callStates[0].effect.settlementPermit, settlement: observation('r1:0') }),
    replacement.transitionProvider('reconcile_provider_request', { turnId: 'r2:turn', requestAttempt: 1 }, request => ({
      ...request, stage: 'settled', effect: responseSettlement.state, settlementEventId: 'r2:settled'
    }))
  ]);
  ...
  await assert.rejects(initial.transitionProvider('consume_provider_settlement', later.identity, request => ({ ...request, stage: 'consumed' })), /does not own/u);
...
test('suspension and cancellation preserve an already-started call and its late permit', async () => {
  for (const phase of [
    ...
    const { runs, initial } = await fixture(runId, [original], [later]);
    const replacement = await runs.attach(runId, 'replacement');
    await replacement.transition(phase.kind === 'cancelling' ? 'finalize_abort' : 'reconcile_tool_call', { phase });
    await initial.settleToolEffect({ effectId: original.callStates[0].effect.intent.effectId,
      permit: original.callStates[0].effect.settlementPermit, settlement: observation('r1:0') });
    const current = await runs.inspect(runId);
    ...
    await assert.rejects(runs.settleToolEffect(runId, {
      ...
      permit: original.callStates[0].effect.settlementPermit, settlement: observation('different')
...
test('global effect conflicts span response groups while independent work may start', async () => {
  ...
  const start = id => initial.transitionTool('start_tool_call', { toolBatchId: id, callIndex: 0 }, call => {
  ...
  await assert.rejects(initial.transition('assemble_turn', state => ({ phase: state.phase, toolBatches: state.toolBatches.slice(1) })), /cannot be removed/u);
});
...
test('source dependencies and the concurrency cap remain enforced at the durable start boundary', async () => {
  ...
  const newer = batch(runId, 'r2', { stage: 'effect_ready', scope: 'new', count: 2, dependencies: [0], maxConcurrency: 1 });
  const { runs, initial } = await fixture(runId, [older, newer]);
  const start = callIndex => initial.transitionTool('start_tool_call', { toolBatchId: 'r2', callIndex }, call => ({
    ...call, stage: 'effect_pending', effect: startExternalEffect(call.effect, call.effect.ticket, 1).state
  }));
...
test('result delivery uncertainty retains the original observation and forbids another send identity', async () => {
  ...
  for (const status of ['admitted', 'submitted', 'uncertain']) await initial.recordToolDelivery(target, { ...identity, status });
  const replacement = await runs.attach(runId, 'replacement');
  await assert.rejects(replacement.recordToolDelivery(target, { ...identity, deliveryId: 'result:A:retry', status: 'admitted' }), /redelivered/u);
  await assert.rejects(replacement.recordToolDelivery(target, { ...identity, status: 'admitted' }), /regress/u);
  await replacement.recordToolDelivery(target, { ...identity, status: 'applied', successorResponseId: 'r3' });
  await assert.rejects(replacement.recordToolDelivery(target, { ...identity, status: 'applied', successorResponseId: 'unrelated' }), /identity cannot change/u);
  assert.deepEqual(replacement.state().toolBatches[0].callStates[0].settlement, observation('r1:0'));
  await settle(runs, runId, work);
  assert.equal((await runs.inspect(runId)).state.toolBatches[0].callStates[0].delivery.successorResponseId, 'r3');
});
...
test('owning codecs reject rewritten original model calls, duplicate effect authorities and the removed observation barrier', async () => {
  ...
  assert.throws(() => decodeAgentRunState({ ...state, toolBatches: [{ ...work, nextObservationIndex: 0 }] }), /Unsupported/u);
  assert.throws(() => decodeAgentRunState({ ...state, toolBatches: [{ ...work, modelCalls: [{ ...work.modelCalls[0], id: 'rewritten' }] }] }), /original model call/u);
  assert.throws(() => decodeAgentRunState({ ...state, toolBatches: [work, { ...work, toolBatchId: 'copy' }] }), /Effect identity/u);
  await assert.rejects(initial.transition('assemble_turn', { phase: state.phase, toolBatches: [{ ...work, source: { ...work.source, responseId: 'rewritten' } }] }), /Original tool source/u);
});
