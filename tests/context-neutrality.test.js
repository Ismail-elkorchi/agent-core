...
import {
  AgentRuntime, AgentSession, AgentRunCoordinator, InferenceService,
  InMemoryInferenceRepository, InMemorySessionRepository, HistoryReader, agentEventCodec
...
const binding = { schemaId: 'neutral-composition', schemaVersion: 1, subject: { application: 'neutral-tests' } };
...
class NeutralProvider extends MemorySimulationProvider {
  ...
  async complete(request) {
    ...
    return { provider: this.id, model: request.model, content: '', terminationReason: 'stop', ...await this.respond(request) };
  }
}
...
async function composition(provider, options = {}) {
  ...
  const runtime = (additional = {}) => new AgentRuntime({
    provider, model: simulationProfile.id, toolBoundary,
    repositories: { artifacts, events, session: { repository, descriptor } },
    maxOutputTokens: 128, limits: { modelTurns: 4, totalToolCalls: 4, consecutiveProviderFailures: 1 },
    ...options, ...additional
  });
...
function assertNeutralCompletion(result) {
  ...
  assert.equal(result.terminal.verificationStatus, 'not_required');
  assert.deepEqual(result.terminal.checkResults, []);
}
...
test('plain assistant retains complete user contributions across runs without a domain workflow', async () => {
  ...
  const app = await composition(provider, { instructions: [
  ...
  for (let i = 0; i < 9; i += 1) assertNeutralCompletion(await app.runtime().run({ task: `An unrelated conversational aside ${i}.` }).result);
  assertNeutralCompletion(await app.runtime().run({ task: 'What bird did I mention?' }).result);
  const request = provider.calls.at(-1);
  assert.equal(request.messages.filter((item) => item.role === 'user' && item.content.includes(original)).length, 1);
  assert.deepEqual(provider.calls[0].messages.filter((item) => item.role === 'system' || item.role === 'developer').map(({ role, content }) => ({ role, content })), [
    { role: 'system', content: system }, { role: 'developer', content: developer }
  ]);
  assert.equal(request.tools?.length ?? 0, 0);
  const history = new HistoryReader({ repository: app.repository, session: app.descriptor, events: app.events, artifacts: app.artifacts });
  const found = await history.search({ query: 'kingfisher', filter: { role: 'user' }, maxBytes: 8000, limit: 5 });
...
test('standalone governed classifier settles structured output with no natural-language answer or interactive run', async () => {
  ...
  const call = { type: 'function', id: 'classification-1', name: 'classify', input: { kind: 'json', value: classification } };
  const provider = new NeutralProvider(() => ({ content: '', toolCalls: [call], output: [{ type: 'tool_call', toolCall: call }], terminationReason: 'tool_calls' }));
  const repository = new InMemoryInferenceRepository();
  const service = new InferenceService({ provider, repository, artifacts: new InMemoryArtifactRepository(), budget: { maxInvocations: 1, maxPromptTokens: 10000, maxCompletionTokens: 256 } });
  const input = {
    invocationId: 'classification-1', ownerId: 'inbox-1', purpose: 'message-classification',
    request: { model: simulationProfile.id, maxOutputTokens: 128, messages: [
      { role: 'developer', content: 'Classify the message using the classify structured result. No prose answer is required.' },
      { role: 'user', content: 'Please help restore my account access.' }
    ], tools: [{ type: 'function', function: { name: 'classify', description: 'Return the classification data.', parameters: { type: 'object', properties: { label: { type: 'string' }, confidence: { type: 'number' } }, required: ['label', 'confidence'], additionalProperties: false } } }] }
  };
...
test('read-only monitor becomes idle until a real external contribution resumes its session', async () => {
  ...
  const readStatus = defineTool({
    name: 'read_status', implementationId: 'neutral-status@1', description: 'Read the current external status.',
    schema: z.strictObject({}), outputSchema: z.strictObject({ status: z.string() }),
    effectEnvelope: access, canonicalizeInput: (input) => input,
    deriveEffects: () => ({ ...access, recovery: { kind: 'unknown' } }),
    invoke: async () => {
      reads += 1;
      return { kind: 'result', ok: true, output: { status: externalStatus }, summary: externalStatus, scope: { resources: ['external-status'], coverage: 'complete' } };
    }
  });
  const provider = new NeutralProvider((request) => {
    if (request.messages.at(-1).role === 'tool') return { content: JSON.stringify({ state: externalStatus }) };
    return { toolCalls: [{ type: 'function', id: `read-${reads + 1}`, name: 'read_status', input: { kind: 'json', value: {} } }], terminationReason: 'tool_calls' };
  });
  const app = await composition(provider, { tools: [readStatus], toolPolicy: { allowedRisks: ['read'] } });
  const session = new AgentSession({
    descriptor: app.descriptor, expectedBinding: binding, repository: app.repository,
    runs: new AgentRunCoordinator(app.events), configuration: { provider: provider.id, model: simulationProfile.id },
    createRuntime: (_configuration, onProgress) => app.runtime({ onProgress })
  });
  const first = await session.submit({ task: 'Read the status once. When there is no new input, wait for an external contribution.' });
  ...
  const next = await session.submit({ task: 'An external contribution has arrived. Read the updated status once.' });
  ...
  for (const runId of [waiting.terminal.runId, ready.terminal.runId]) {
    ...
    assert.ok(entries.filter((event) => event.type === 'tool.started').every((event) => event.effects.accesses.every((access) => access.mode === 'read')));
    assert.equal(entries.filter((event) => event.type === 'check.started').length, 0);
    for (const event of entries.filter((event) => event.type === 'run.disposition.decided')) assert.deepEqual(event.decision, { kind: 'accept' });
  }
});
