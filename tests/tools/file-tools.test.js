...
test('directory and path selection are sorted, Git-aware, hidden-safe, and match root globs', async () => {
  ...
  assert.deepEqual(
    listed.output.entries.map((entry) => entry.path),
    [...listed.output.entries.map((entry) => entry.path)].sort((a, b) => a.localeCompare(b, 'en'))
  );
  assert.equal(
    listed.output.entries.some((entry) => entry.path === 'ignored.txt'),
    false
  );
  assert.equal(
    listed.output.entries.some((entry) => entry.path === '.secret'),
    false
  );
  assert.equal(listed.output.counts.omitted.count > 0, true);
  assert.equal(listed.output.counts.omitted.relation, 'exact');
  assert.equal(
    listed.output.omissionSamples.some((entry) => entry.reason === 'hidden'),
    true
  );
  assert.equal(
    listed.output.omissionSamples.some((entry) => entry.reason === 'gitignored'),
    true
  );
  ...
  const rootGlob = await invokeToolCall(
    jsonToolCall('find_files', { patterns: ['*.ts'] }),
    tools,
    context
  );
  assert.equal(rootGlob.kind, 'result');
  assert.deepEqual(
    rootGlob.output.entries.map((entry) => entry.path),
    ['a.ts', 'z.ts']
  );

  const excluded = await invokeToolCall(
    jsonToolCall('list_directory', { exclude: ['*.tmp'] }),
    tools,
    context
  );
  assert.equal(
    excluded.output.entries.some((entry) => entry.path === 'skip.tmp'),
    false
  );
  assert.equal(
    excluded.output.omissionSamples.some(
      (entry) => entry.path === 'skip.tmp' && entry.reason === 'excluded'
    ),
    true
  );
  ...
  const shallowContext = {
    ...
    services: {
      ...
      rootedFileSelector: new RootedFileSelector(
        context.services.rootedFileAuthority,
        shallowConfiguration.fileSelection
      )
    }
  };
  const clampedDepth = await invokeToolCall(
    jsonToolCall('list_directory', { depth: 99 }),
    tools,
    shallowContext
  );
  assert.equal(
    clampedDepth.output.entries.some((entry) => entry.path === 'src/b.ts'),
    false
  );

  const hidden = await invokeToolCall(
  ...
  assert.equal(
    hidden.output.entries.some((entry) => entry.path === '.secret'),
    true
  );
  assert.equal(
    hidden.output.entries.some((entry) => entry.path === '.hidden/deep.ts'),
    true
  );
  assert.equal(
    hidden.output.entries.some((entry) => entry.path === 'ignored.txt'),
    true
  );

  const hiddenTypescript = await invokeToolCall(
    jsonToolCall('find_files', {
      patterns: ['**/*.ts'],
      includeHidden: true,
      respectGitIgnore: false
  ...
  assert.equal(
    hiddenTypescript.output.entries.some((entry) => entry.path === '.hidden/deep.ts'),
    true
  );
  assert.equal(
    hiddenTypescript.output.entries.every((entry) => entry.path.endsWith('.ts')),
    true
  );
});
...
test('local tool hosts retain an owned configuration snapshot after caller mutation', async () => {
  ...
  const localToolConfiguration = requireLocalToolConfiguration({
    services: { localToolConfiguration: callerOwned }
  });
  const context = {
    policy,
    services: { rootedFileAuthority: testRootedFileAuthority(root), localToolConfiguration }
  };

  callerOwned.readFiles.maxFiles = 1;
  callerOwned.readFiles.unexpected = 10;
  const observation = await invokeToolCall(
    jsonToolCall('read_files', { files: [{ path: 'one.txt' }, { path: 'two.txt' }] }),
    [readFilesTool],
    context
  );
  assert.equal(observation.output.returnedFiles, 2);
  assert.equal(
    localToolConfiguration.readFiles.maxFiles,
    DEFAULT_LOCAL_TOOL_CONFIGURATION.readFiles.maxFiles
  );
  ...
  assert.throws(
    () => requireLocalToolConfiguration({ services: { localToolConfiguration: invalid } }),
...
test('directory traversal continues with partial coverage after an unreadable branch', async (t) => {
  ...
  assert.equal(
    result.output.entries.some((entry) => entry.path === 'readable/kept.txt'),
    true
  );
  if (process.getuid !== undefined && process.getuid() !== 0) {
    assert.equal(result.output.coverage, 'partial');
    assert.equal(
      result.output.omissionSamples.some((entry) => entry.reason === 'unreadable'),
      true
    );
  } else {
    assert.equal(result.output.coverage, 'complete');
    assert.equal(
      result.output.entries.some((entry) => entry.path === 'unreadable/hidden.txt'),
      true
    );
  }
});
...
test('list_directory depth is structural and find_files still traverses to the host depth', async () => {
  ...
  const shallow = await invokeToolCall(
    jsonToolCall('list_directory', { depth: 1 }),
    tools,
    shallowContext
  );
  assert.deepEqual(
    shallow.output.entries.map((entry) => entry.path),
    ['a-deep', 'top.txt', 'z-other']
  );
  ...
  const hostContext = {
    ...
    services: {
      ...context.services,
      localToolConfiguration: hostConfiguration,
      rootedFileSelector: new RootedFileSelector(context.services.rootedFileAuthority, hostLimited)
    }
  };
  const requestedDeeper = await invokeToolCall(
    jsonToolCall('list_directory', { depth: 3 }),
    tools,
    hostContext
  );
  ...
  assert.equal(
    requestedDeeper.output.entries.some((entry) => entry.path === 'a-deep/first.txt'),
    false
  );

  const found = await invokeToolCall(
    jsonToolCall('find_files', { patterns: ['**/*.txt'] }),
    tools,
    context
  );
  assert.deepEqual(
    found.output.entries.map((entry) => entry.path),
    ['a-deep/branch/deep.txt', 'a-deep/first.txt', 'top.txt', 'z-other/second.txt']
  );
  const hostFound = await invokeToolCall(
    jsonToolCall('find_files', { patterns: ['**/*.txt'] }),
    tools,
    hostContext
  );
...
test('read_files streams large ranges, preserves batch failures, and hashes raw selected bytes', async () => {
  ...
  const result = await invokeToolCall(
    jsonToolCall('read_files', {
      files: [
        { path: 'large.txt', startLine, lineCount: 2 },
        { path: 'raw.txt', lineCount: 1 },
        { path: 'missing.txt' }
      ]
    }),
    tools,
  ...
  assert.equal(
    result.output.files[1].rangeSha256,
    createHash('sha256').update(Buffer.from('one\r\n')).digest('hex')
  );
  assert.equal(result.output.files[1].newlineConvention, 'crlf');
  assert.equal(
    result.output.files[1].fullFileSha256,
    createHash('sha256').update(Buffer.from('one\r\ntwo\r\n')).digest('hex')
  );
  assert.equal(result.output.files[1].truncated, true);
  assert.equal(result.output.files[1].utf8Validation, 'valid');
  assert.deepEqual(
    result.output.failures.map((failure) => failure.reason),
    ['not_found']
  );

  const full = await invokeToolCall(
    jsonToolCall('read_files', { files: [{ path: 'raw.txt', lineCount: 10 }] }),
    tools,
    context
  );
  ...
  assert.equal(
    full.output.files[0].fullFileSha256,
    createHash('sha256').update(Buffer.from('one\r\ntwo\r\n')).digest('hex')
  );
});
...
test('search_text delegates regex validation to ripgrep and separates line and occurrence counts', async () => {
  ...
  const result = await invokeToolCall(
    jsonToolCall('search_text', { query: 'foo', mode: 'matches' }),
    tools,
    context
  );
  assert.equal(result.kind, 'result');
  ...
  const counts = await invokeToolCall(
    jsonToolCall('search_text', { query: 'foo', mode: 'count' }),
    tools,
    context
  );
  assert.deepEqual(
    counts.output.results.map((entry) => [
      entry.path,
      entry.matchingLineCount,
  ...
  const limitedFiles = await invokeToolCall(
    jsonToolCall('search_text', { query: 'foo', mode: 'files', resultLimit: 1 }),
    tools,
    context
  );
  ...
  const invalid = await invokeToolCall(
    jsonToolCall('search_text', { query: '[', mode: 'files' }),
    tools,
    context
  );
  ...
  const invalidWithoutFiles = await invokeToolCall(
    jsonToolCall('search_text', { query: '[', path: 'empty', mode: 'files' }),
    tools,
    context
  );
...
test('nested gitignore negation, ignore limits, and visit limits are deterministic', async () => {
  ...
  await Promise.all(
    Array.from({ length: 10 }, (_unused, index) =>
      writeFile(path.join(root, `file-${String(index).padStart(2, '0')}.txt`), 'x')
    )
  );
  const nested = await invokeToolCall(
    jsonToolCall('find_files', { patterns: ['**/*'], includeHidden: true }),
    tools,
    context
  );
  assert.equal(
    nested.output.entries.some((entry) => entry.path === 'nested/a.log'),
    false
  );
  assert.equal(
    nested.output.entries.some((entry) => entry.path === 'nested/important.log'),
    true
  );
  ...
  const limitedConfiguration = {
    ...DEFAULT_LOCAL_TOOL_CONFIGURATION,
    fileSelection: {
      ...DEFAULT_LOCAL_TOOL_CONFIGURATION.fileSelection,
      maxVisitedEntries: 3,
      maxIgnoreFiles: 1
    }
  };
  const limitedContext = {
    ...
    services: {
      ...context.services,
      localToolConfiguration: limitedConfiguration,
      rootedFileSelector: new RootedFileSelector(
        context.services.rootedFileAuthority,
        limitedConfiguration.fileSelection
      )
    }
  };
...
test('read_files reports precise EOF, UTF-8, and batch limit outcomes', async () => {
  ...
  const edge = await invokeToolCall(
    jsonToolCall('read_files', {
      files: [{ path: 'short.txt', startLine: 9 }, { path: 'invalid.txt' }, { path: 'short.txt' }]
    }),
    tools,
    context
  );
  assert.deepEqual(
    edge.output.failures.map((failure) => failure.reason),
    ['start_after_eof', 'invalid_utf8']
  );
  ...
  const limitedConfiguration = {
    ...DEFAULT_LOCAL_TOOL_CONFIGURATION,
    readFiles: { ...DEFAULT_LOCAL_TOOL_CONFIGURATION.readFiles, maxFiles: 1, maxTotalBytes: 4 }
  };
  const limitedContext = {
    ...context,
    services: { ...context.services, localToolConfiguration: limitedConfiguration }
  };
  const limited = await invokeToolCall(
    jsonToolCall('read_files', { files: [{ path: 'short.txt' }, { path: 'second.txt' }] }),
    tools,
    limitedContext
  );
  assert.equal(
    limited.output.failures.some((failure) => failure.reason === 'batch_byte_limit'),
    true
  );
  assert.equal(
    limited.output.failures.some((failure) => failure.reason === 'batch_file_limit'),
    true
  );
});
...
test('read_files handles empty files, unterminated final lines, and UTF-8 split across scan chunks', async () => {
  ...
  assert.equal(
    result.output.files[0].fullFileSha256,
...
test('read_files detects replacement, growth, and truncation of an opened file', async () => {
  for (const mutation of ['replace', 'grow', 'truncate']) {
    ...
    const configuration = {
      ...DEFAULT_LOCAL_TOOL_CONFIGURATION,
      readFiles: {
        ...DEFAULT_LOCAL_TOOL_CONFIGURATION.readFiles,
    ...
    const result = await invokeToolCall(
      jsonToolCall('read_files', { files: [{ path: 'changing.txt' }] }),
      tools,
      { ...changingContext, emitProgress }
    );
    assert.equal(mutated, true, mutation);
    assert.equal(result.kind, 'result');
    assert.equal(
      result.output.failures.some((failure) => failure.reason === 'file_changed'),
      true,
      mutation
    );
  }
...
test('read_files observes abort during a long streamed scan', async () => {
  ...
  const promise = invokeToolCall(
    jsonToolCall('read_files', { files: [{ path: 'long.txt' }] }),
    tools,
    {
      ...context,
      ...
      emitProgress(progress) {
        if (progress.stage === 'file_reading') controller.abort('cancel long read');
      }
    }
  );
...
test('search_text handles long repositories, context, per-file limits, abort, and missing ripgrep', async () => {
  ...
  const long = await invokeToolCall(
  ...
  assert.deepEqual(long.output.perFileOmissions, [
    {
      path: long.output.results[0].path,
      cause: 'per_file_limit',
      retainedMatches: 1,
      omittedAtLeast: 1
    }
  ]);
  ...
  const planningContext = {
    ...context,
    signal: controller.signal,
    boundary: { authorizationPolicyId: 'tests/search@1', executionTargetId: root }
  };
  ...
  try {
    process.env.PATH = '';
    const missing = await invokeToolCall(
      jsonToolCall('search_text', { path: 'many', query: 'needle' }),
      tools,
      context
    );
...
test('search_text reports independent per-file omissions in matches and files modes while count mode stays exact', async () => {
  ...
  const matches = await invokeToolCall(
    jsonToolCall('search_text', { query: 'needle', mode: 'matches', perFileLimit: 2 }),
    tools,
    context
  );
  ...
  assert.deepEqual(
    matches.output.perFileOmissions.map((item) => [
      item.path,
      item.cause,
      item.retainedMatches,
      item.omittedAtLeast
    ]),
    [
      ['above.txt', 'per_file_limit', 2, 1],
      ['second-above.txt', 'per_file_limit', 2, 1]
    ]
  );
  assert.deepEqual(
    Object.fromEntries(
      matches.output.results.reduce(
        (map, item) => map.set(item.path, (map.get(item.path) ?? 0) + 1),
        new Map()
      )
    ),
    {
  ...
  const counts = await invokeToolCall(
    jsonToolCall('search_text', { query: 'needle', mode: 'count', perFileLimit: 2 }),
    tools,
    context
  );
  ...
  assert.deepEqual(
    counts.output.results.map((item) => [item.path, item.matchingLineCount, item.occurrenceCount]),
    [
      ['above.txt', 3, 3],
      ['at.txt', 2, 2],
      ['below.txt', 1, 1],
      ['second-above.txt', 4, 4]
    ]
  );

  const files = await invokeToolCall(
    jsonToolCall('search_text', { query: 'needle', mode: 'files', perFileLimit: 1 }),
    tools,
    context
  );
  ...
  assert.deepEqual(
    files.output.perFileOmissions.map((item) => item.path),
    ['above.txt', 'at.txt', 'second-above.txt']
  );
});
...
test('search_text bounds and parses multi-megabyte ripgrep output without quadratic truncation', async () => {
  ...
  const largeConfiguration = {
    ...DEFAULT_LOCAL_TOOL_CONFIGURATION,
    searchText: {
      ...DEFAULT_LOCAL_TOOL_CONFIGURATION.searchText,
      maxResults: 3_000,
      maxOutputBytes: 6 * 1024 * 1024,
      maxFileBytes: 8 * 1024 * 1024
    }
  };
  const large = await invokeToolCall(
    jsonToolCall('search_text', {
      query: 'needle',
      mode: 'matches',
      resultLimit: 3_000,
    ...
    {
      ...context,
      services: { ...context.services, localToolConfiguration: largeConfiguration }
    }
  );
  ...
  const bounded = await invokeToolCall(
    jsonToolCall('search_text', {
      query: 'needle',
      mode: 'matches',
      resultLimit: 3_000,
    ...
    {
      ...context,
      services: { ...context.services, localToolConfiguration: boundedConfiguration }
    }
  );
