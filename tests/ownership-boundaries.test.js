...
test('agent event append, idempotency, hashing, and read share one owned value', async () => {
  ...
  const observation = parseToolObservation(
    { outputSchema: z.strictObject({ nested: z.strictObject({ count: z.number() }) }) },
    observationInput
  );
  const committed = await new ObservationStore().commitToolObservation({
    turnIndex: 1,
    call: { id: 'call', name: 'tool', input: { kind: 'json', value: {} } },
    observation
  });
  const original = {
    type: 'tool.ended',
    ...
    toolAttempt: 1,
    toolName: 'tool',
    observation: committed.original
  };
  const encoded = encodeAgentEvent(original);
  const appended = await repository.append('run-1', original, {
  ...
  const duplicate = await repository.append('run-1', original, {
    timestamp: '2026-01-01T00:00:00.000Z',
    idempotencyKey: 'same'
  });
  assert.deepEqual(duplicate, appended);
  const [read] = await Array.fromAsync(repository.read('run-1'));
  assert.equal(read.event.observation.observation.output.nested.count, 1);
  assert.throws(() => {
    read.event.observation.observation.output.nested.count = 4;
  }, TypeError);
...
test('plain errors produce deeply immutable invalid-output observations', () => {
  ...
  assert.throws(
    () => observation.output.issues.issues.push({ path: [], code: 'changed', message: 'changed' }),
    TypeError
  );
  assert.throws(() => {
    observation.output.issues.issues[0].message = 'changed';
  }, TypeError);
...
test('tool planning accepts owned calls without decoding structural lookalikes', async () => {
  const tool = defineTool({
    name: 'planned_tool',
    implementationId: 'tests/plan-tool@1',
    description: 'Checks call ownership.',
    schema: z.strictObject({ value: z.string() }),
  ...
  const context = {
    ...
    boundary: { authorizationPolicyId: 'tests', executionTargetId: 'tests' }
  };
...
test('trusted model request construction owns retained input before provider validation', () => {
  const message = {
    role: 'user',
    content: 'before',
    images: [{ type: 'bytes', data: new Uint8Array([1]), mediaType: 'image/png' }]
  };
...
test('owned extension observations are not decoded again', () => {
  let outputChecks = 0;
  const outputSchema = z.unknown().superRefine(() => {
    outputChecks += 1;
  });
  const tool = { outputSchema };
  const owned = parseToolObservation(tool, {
    kind: 'result',
    summary: 'ok',
    scope: { resources: [], coverage: 'complete' },
    output: { value: 1 }
  ...
  assert.throws(
    () =>
      agentEventCodec.decode({
        ...
        id: 'observation-1',
        toolName: 'read',
        call: { name: 'read', input: { kind: 'json', value: {} } },
        ...
        modelContent: [{ type: 'text', text: 'Read a resource.' }]
      }),
    /uri/iu
  );
});
