...
export type AgentDispositionPhase =
  ...
  | Readonly<
      AgentDispositionPhaseBase & {
        readonly stage: 'effect_ready';
        readonly plan: AgentDispositionEffectPlanRecord;
...
export function decodeDispositionPhase(value: unknown): AgentDispositionPhase {
  ...
  const stage = enumeration(
    phase.stage,
    ['ready', 'effect_ready', 'effect_pending', 'outcome_unknown', 'decided'] as const,
    'disposition.stage'
  );
  const base = {
    kind: 'disposition' as const,
    identity: decodeTurnIdentity(phase.identity),
    providerSettlementEventId: identifier(
      phase.providerSettlementEventId,
      'disposition.providerSettlementEventId'
    ),
  ...
  if (base.budgetSnapshot.revisionAttempts !== base.revisionCount)
    throw new TypeError('Disposition budget snapshot contradicts its revision count.');
  if (stage === 'ready') {
  ...
  if (stage === 'decided') {
    ...
    if (effect !== undefined && effect.phase !== 'settled')
      throw new TypeError('A decided disposition may retain only a settled effect.');
    ...
    if (hashJson(decision) !== outputDigest)
      throw new TypeError('Disposition output digest does not match its decision.');
    if (
      effect &&
      (effect.settlement.outcome !== 'succeeded' ||
  ...
  if (
    effect.intent.implementationId !== plan.implementationId ||
  ...
  if (stage === 'effect_ready') {
    if (effect.phase !== 'ticket_issued')
      throw new TypeError('An effect-ready disposition requires an issued ticket.');
    return Object.freeze({ ...base, stage, plan, effect });
  }
  if (stage === 'effect_pending') {
    if (effect.phase !== 'started')
      throw new TypeError('An effect-pending disposition requires a started effect.');
    return Object.freeze({ ...base, stage, plan, effect });
  }
  if (effect.phase !== 'started' && effect.phase !== 'closed')
    throw new TypeError('An unknown disposition outcome requires a started or closed effect.');
  if (effect.phase === 'closed' && effect.closure.reason === 'cancelled_before_start')
    throw new TypeError('An unknown disposition effect must have been started.');
  return Object.freeze({ ...base, stage, plan, effect });
}
...
function decodeEffectPlan(value: unknown, inputDigest: string): AgentDispositionEffectPlanRecord {
  ...
  const fingerprint = hashJson(
    Object.freeze({
      implementationId: decoded.implementationId,
      inputDigest,
  ...
  if (fingerprint !== decoded.fingerprint)
    throw new TypeError('Disposition plan fingerprint does not match its captured intent.');
  return decoded;
}
...
function decodeControlSnapshot(
  value: unknown
): Readonly<{ readonly status: 'owned'; readonly driverGeneration: number }> {
  ...
  return Object.freeze({
    ...
    driverGeneration: nonnegativeInteger(control.driverGeneration, 'disposition control driverGeneration')
  });
}
...
function object(value: unknown, label: string): JsonObject {
  try {
    ...
    throw new TypeError(`${label} must be an object.`, { cause: error });
  }
}
...
function exact(value: JsonObject, fields: readonly string[]): void {
  ...
  if (unsupported.length > 0)
    throw new TypeError(`Disposition state has unsupported fields: ${unsupported.join(', ')}.`);
}
...
function enumeration<const T extends readonly string[]>(
  ...
  if (typeof value !== 'string' || !values.some((modelOutput) => modelOutput === value))
