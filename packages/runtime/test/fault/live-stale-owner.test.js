...
for (const mode of modes) {
  test(
    `a live stale owner cannot duplicate work when paused ${mode.replaceAll('_', ' ')}`,
    { timeout: 30_000 },
    async () => {
      ...
      try {
        ...
        assert.match(
          oldOutcome.message,
          /driver|stale|tail|replacement|terminal run|already terminal|idempotency_conflict/iu
        );
        ...
        assert.equal(
          records.filter((event) => event.type === 'observation.record.created').length,
          1
        );
...
function exitCode(child) {
  ...
  return new Promise((resolve, reject) => {
    ...
    child.once('exit', (code, signal) =>
      signal === null ? resolve(code) : reject(new Error(`Fixture exited with ${signal}.`))
    );
  });
}
