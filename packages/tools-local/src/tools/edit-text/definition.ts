...
export const editTextTool = defineTool({
  ...
  canonicalizeInput(input, context): CanonicalEditTextInput {
    ...
    if (input.files.length > limits.maxFiles)
      throw new ToolInputError(
        `edit_text accepts at most ${String(limits.maxFiles)} files per transaction.`
      );
    const files = input.files.map((file) => {
      if (file.edits.length > limits.maxEditsPerFile)
        throw new ToolInputError(
          `File ${file.path} exceeds the host limit of ${String(limits.maxEditsPerFile)} localized edits.`
        );
        ...
        edits: Object.freeze(
          file.edits.map((edit) =>
            Object.freeze({
              ...edit,
              range: Object.freeze({
                start: Object.freeze({ ...edit.range.start }),
                end: Object.freeze({ ...edit.range.end })
              })
            })
          )
        )
    ...
    if (duplicates.length > 0)
      throw new ToolInputError(
        `edit_text file paths must be unique after normalization: ${duplicates.join(', ')}.`,
        { duplicatePaths: duplicates }
      );
    const replacementBytes = files.reduce(
      (total, file) =>
        total +
        file.edits.reduce((sum, edit) => sum + Buffer.byteLength(edit.replacementText, 'utf8'), 0),
      0
    );
  ...
  deriveEffects(input, context) {
    const accesses = input.files.flatMap((file) =>
      input.dryRun
        ? [{ mode: 'read' as const, scope: fileScope(file.path) }]
        : [
            { mode: 'read' as const, scope: fileScope(file.path) },
            { mode: 'write' as const, scope: fileScope(file.path) }
          ]
    );
    ...
    return {
      accesses,
      lockScopes: [
        ...new Set([...input.files.map((file) => fileScope(file.path)), PATCH_JOURNAL_SCOPE])
      ].sort(),
      recovery: {
