...
const storedEvents = new JsonlEventRepository({
  rootDir: path.join(root, 'events'),
  codec: agentEventCodec
});
const events =
  mode === 'crash_after_ended' ||
  mode === 'crash_before_started' ||
  mode === 'crash_before_recording'
    ? {
        async append(...args) {
        ...
        async appendConditional(...args) {
          const batches =
            args[1]?.type === 'run.state.transitioned' && args[1].transition.kind === 'updated'
              ? (args[1].transition.toolRecords ?? []).map((entry) => entry.value)
              : [];
          if (
            mode === 'crash_before_started' &&
            batches.some((batch) => batch.calls.some((call) => call.stage === 'effect_pending'))
          )
            process.exit(45);
          const result = await storedEvents.appendConditional(...args);
          if (
            mode === 'crash_after_ended' &&
            batches.some((batch) => batch.calls.some((call) => call.stage === 'settled'))
          )
            process.exit(43);
          if (
            mode === 'crash_before_recording' &&
            batches.some((batch) => batch.calls.some((call) => call.stage === 'recording'))
          )
            process.exit(47);
          return result;
        },
        readRange: (...args) => storedEvents.readRange(...args),
        readReference: (...args) => storedEvents.readReference(...args),
        referenceByKey: (...args) => storedEvents.referenceByKey(...args),
        latestReferenceOfType: (...args) => storedEvents.latestReferenceOfType(...args),
...
const recoveryKind = await readFile(path.join(root, 'recovery-kind.txt'), 'utf8').then(
  (value) => value.trim(),
  () => 'unknown'
);
const session =
...
const provider = {
  ...
  describe() {
    return { id: 'fixture', displayName: 'Fixture', defaultModel: 'fixture' };
  },
  async describeModel() {
    return {
      id: 'fixture',
  ...
  async complete() {
    if (mode === 'suspend')
      return {
        ...
        toolCalls: [
          { id: 'effect-1', type: 'function', name: 'effect', input: { kind: 'json', value: {} } }
        ]
      };
    return {
      content: 'unexpected replay completed',
      model: 'fixture',
      provider: 'fixture',
...
const tool = {
  name: 'effect',
  implementationId: 'tests/crash-effect@1',
  description: 'writes one externally visible marker',
  jsonSchema: { type: 'object' },
  outputSchema: z.strictObject({}),
  effectEnvelope: {
    accesses: [
      { mode: 'read', scope: 'fixture/source' },
      { mode: 'write', scope: 'fixture/effect' }
    ],
    lockScopes: ['fixture/effect']
  },
  decodeInput() {
  ...
  async deriveEffects() {
    if (recoveryKind === 'preconditioned') {
      return {
        accesses: [{ mode: 'read', scope: 'fixture/source' }],
        lockScopes: [],
        recovery: {
          ...
          preconditions: [
            {
              ...
              validatorId: 'tests/source-sha256@1',
              expectedVersion: await sourceVersion()
            }
          ]
        }
      };
    ...
    if (recoveryKind === 'buffered') {
      return {
        accesses: [{ mode: 'write', scope: 'fixture/effect' }],
        lockScopes: ['fixture/effect'],
        recovery: {
          kind: 'buffered_mutation',
          authority: 'tests/fixture-journal@1',
          reconcilerId: 'tests/fixture-reconciler@1',
          transactionId: 'effect-1'
        }
      };
    }
    return {
  ...
  bindExecution(input) {
    return {
      ...
      async invoke(context) {
        if (recoveryKind === 'buffered')
          await appendFile(
            path.join(root, 'buffered-receipt.json'),
            JSON.stringify({ transactionId: 'effect-1' })
          );
        if (recoveryKind === 'preconditioned') {
          const expected = context.invocation?.recovery?.preconditions[0]?.expectedVersion;
          if (context.invocation?.toolAttempt > 1 && (await sourceVersion()) !== expected)
            throw new Error('Recovered read precondition changed before invocation.');
        ...
        return {
          kind: 'result',
          output: {},
          summary: 'effect happened again',
          scope: { resources: ['fixture/effect'], coverage: 'complete' }
        };
      },
      async recover(effect) {
        ...
        if (effect.intent.recovery.kind === 'buffered_mutation') {
          const receipt = await readFile(path.join(root, 'buffered-receipt.json'), 'utf8').then(
            JSON.parse,
            () => undefined
          );
          return receipt?.transactionId === effect.intent.recovery.transactionId
            ? {
                ...
                observation: {
                  kind: 'result',
                  output: {},
                  summary: 'reconciled buffered mutation',
                  scope: { resources: ['fixture/effect'], coverage: 'complete' }
                }
              }
...
async function sourceVersion() {
  return createHash('sha256')
    .update(await readFile(path.join(root, 'source.txt')))
    .digest('hex');
}
...
const agent = new AgentRuntime({ maxOutputTokens: 64,
  ...
  onProgress(event) {
    if (
      mode === 'crash_waiting_for_lease' &&
      event.type === 'tool.updated' &&
      event.progress.stage === 'resource_lease_waiting'
    )
      process.exit(44);
  }
});
...
if (mode === 'suspend') {
  const result = await agent.run({ task: 'crash after approved effect with unknown recovery' })
    .result;
  ...
  const result =
    mode === 'recover'
      ...
      : await (
          await agent.resolveApproval({ runId, approvalId, fingerprint, decision: 'allow' })
        ).result;
