...
test('1,000 completed inputs retain exact conversation until an explicit context change', { timeout: 180_000 }, async (context) => {
  ...
  const provider = {
    ...
    describe: () => ({ id: profile.provider, displayName: 'Deterministic conversation fixture', defaultModel: profile.id }),
    describeModel: async () => profile,
    async complete(request) {
      const count = accepted.length;
      if (boundaries.has(count)) {
        const originalInputs = request.messages.filter((item) => item.role === 'user').map((item) => item.content);
        for (const input of accepted) {
          assert.equal(originalInputs.filter((content) => content === input).length, 1,
            `Original input must appear exactly once at conversation boundary ${String(count)}.`);
        }
        const replies = request.messages.filter((item) => item.role === 'assistant').map((item) => item.content);
        for (let index = 1; index < count; index++) {
          assert.equal(replies.filter((content) => content === `acknowledged:${String(index)}`).length, 1);
        }
        inspected.push(count);
      }
      return {
        provider: profile.provider, model: profile.id,
        content: `acknowledged:${String(count)}`, terminationReason: 'stop',
