...
const profile = {
  ...
  limits: { contextTokens: 200000, maxInputTokens: 196000, outputTokens: 4000 },
      ...
      toolCalls: [
        {
          ...
          type: 'function',
          name: 'write',
          input: { kind: 'json', value: { data: value, signature: value } }
        }
      ]
    }
  ],
...
test('provider input identities validate without invoking accessors and are independent of object key order', async () => {
  ...
  assert.equal(
    await modelInputIdentity({ z: 1, a: ['hello'] }),
    await modelInputIdentity({ a: ['hello'], z: 1 })
  );
...
test('opaque state never becomes zero or ciphertext-byte token accounting', async () => {
  ...
  const logical = {
    ...base,
    messages: [...base.messages, { role: 'protocol', content: '', state }],
    maxOutputTokens: 100
  };
  ...
  const logical = {
    ...
    messages: [
      { role: 'developer', content: 'authority' },
      {
        role: 'user',
        content: '',
        parts: [{ type: 'audio', mediaType: 'audio/wav', source: { type: 'base64', value: 'abc' } }]
      },
      {
        role: 'control',
        content: '',
        update: { id: 'cfg-1', type: 'configuration', reasoning: { strategy: 'effort', effort: 'high' } }
      }
    ],
    tools: [
      { type: 'function', function: { name: 'tool', parameters: { description: 'x'.repeat(5000) } } }
    ],
    responseFormat: { type: 'json_schema', schema: { description: 'x'.repeat(5000) } },
...
test('compiled OpenAI request owns the exact admitted body and preserves native roles', async () => {
  ...
  const provider = new OpenAIProvider({
    ...
    fetch: async (_url, init) => {
      ...
      return Response.json({ id: 'r', model: 'gpt-5.6-sol', status: 'completed', output_text: 'done' });
    }
  });
  const original = {
    model: 'gpt-5.6-sol',
  ...
  assert.throws(() => {
    compiled.body.input[0].content = 'mutated';
  }, TypeError);
    ...
    messages: [
      {
        role: 'user',
        content: '',
        parts: [{ type: 'image', image: { type: 'bytes', mediaType: 'image/png', data: source } }]
      }
    ]
  };
...
test('missing output reservation is explicit instead of a silent zero budget', () => {
  const accounting = accountModelRequest(
...
test('context admission includes separate reasoning and rejects unknown reasoning semantics', () => {
  const request = {
    ...
    maxOutputTokens: 100,
    reasoning: { strategy: 'budget', maxTokens: 50 }
  };
  const counted = { providerInputTokens: 100 };
  const definition = (semantics) => ({
    ...profile,
    limits: { contextTokens: 240, outputTokens: 100 },
    capabilities: { ...profile.capabilities, protocol: { reasoningAccounting: semantics } }
  });
  assert.throws(
    () => assertRequestAccountingFits(accountModelRequest(request, definition('separate'), counted)),
    /Context token limit/u
  );
  assert.doesNotThrow(() =>
    assertRequestAccountingFits(accountModelRequest(request, definition('included_output'), counted))
  );
  assert.throws(
    () => assertRequestAccountingFits(accountModelRequest(request, definition('unknown'), counted)),
    /Reasoning accounting semantics/u
  );
});
...
test('Responses provider counting resolves preserved encrypted reasoning cost without counting ciphertext bytes', async () => {
  ...
  const native = {
    type: 'reasoning',
    id: 'rs-exact',
    encrypted_content: 'SIGNED+/='.repeat(1000),
