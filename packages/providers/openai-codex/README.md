...
This transport rejects a requested generation cap. `outputReservation` is therefore an explicit host reservation policy, not a server-enforced cap. Without a policy, compiled accounting marks output reservation unknown and governed admission must resolve it. Never reserve zero by default or equate encrypted state bytes with logical tokens.
