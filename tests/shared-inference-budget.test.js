...
test('primary and auxiliary inference consume the same durable owner budget', async () => {
  let calls = 0;
  const profile = {
    id: 'shared-budget', provider: 'shared-budget',
    capabilities: {
  ...
  const provider = {
    ...
    describe: () => ({ id: profile.provider, displayName: 'Shared budget fixture', defaultModel: profile.id }),
    describeModel: async () => profile,
    async complete() {
      calls++;
      return {
        provider: profile.provider, model: profile.id, content: 'Complete.', terminationReason: 'stop',
  ...
  const createService = () => new InferenceService({ provider, repository, artifacts, budget: { maxInvocations: 1 } });
  ...
  const result = await runtime.run({ runId: ownerId, task: 'Complete the primary request.' }).result;
  ...
  await assert.rejects(createService().invoke({
    invocationId: 'auxiliary-after-primary', ownerId, purpose: 'verification',
    request: { model: profile.id, messages: [{ role: 'user', content: 'Verify the result.' }], maxOutputTokens: 64 }
  }), InferenceBudgetExceededError);
  ...
  const independent = await createService().invoke({
    invocationId: 'independent-request', ownerId: 'another-owner', purpose: 'classification',
    request: { model: profile.id, messages: [{ role: 'user', content: 'Classify this input.' }], maxOutputTokens: 64 }
  });
