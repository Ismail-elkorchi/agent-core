...
const acceptance = (runId = 'run-run') => ({
  ...
  configuration: {
    providerId: 'scripted',
    providerImplementationId: 'agent-core.tests.run-provider@1',
...
test('run acceptance is durable and inspection is read-only', async () => {
  ...
  const before = await events.tail('run-run');
  const inspected = await runs.inspect('run-run');
  assert.deepEqual(await events.tail('run-run'), before);
...
test('driver attachment fences a live stale owner and all writes retain one tail authority', async () => {
  ...
  const staleDrive = first.drive(async ({ instruction }) => {
    ...
    await blocked;
    return { phase: { kind: 'initializing', step: 'assemble_turn', turnIndex: 1 } };
  });
  ...
  assert.equal(advanced.inspection.state.phase.kind, 'initializing');
});
...
test('abort retries a lost tail race and becomes the durable control state', async () => {
  ...
  const results = await Promise.allSettled([
    driver.drive(() => ({ phase: { kind: 'initializing', step: 'assemble_turn', turnIndex: 1 } })),
    runs.requestAbort('abort-race', 'User requested cancellation.')
  ]);
...
test('total run states select one explicit procedure, wait, or completion', () => {
  const accepted = decodeAgentRunState({
