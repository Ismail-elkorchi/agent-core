...
for (const [name, create] of [
  ...
  test(`${name} event repository conditionally commits against tail and driver generation`, async () => {
    ...
    const staleTail = await repository.appendConditional('conditional', { type: 'run.state.transitioned' }, {
    ...
    const staleDriver = await repository.appendConditional('conditional', { type: 'run.state.transitioned' }, {
    ...
    const transitioned = await repository.appendConditional('conditional', { type: 'run.state.transitioned', revision: 1 }, {
    ...
    assert.equal((await repository.latestOfType('conditional', 'run.state.transitioned')).event.revision, 1);
  });
}
