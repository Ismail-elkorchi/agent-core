...
const binding = Object.freeze({
  schemaId: 'agent-core.tests/self-hosting',
  schemaVersion: 1,
  subject: Object.freeze({ application: 'self-hosting-smoke' })
});

test(
  'scripted self-hosting run survives approvals, structured tools, and one terminal commit',
  ...
  async () => {
    ...
    assert.deepEqual(
      result.pendingApprovals.map((approval) => approval.toolName),
      ['apply_patch']
    );
    for (;;) {
      const approval = result.pendingApprovals[0];
      result = await (
        await new AgentRuntime(options).resolveApproval({
          runId: result.runId,
    ...
    assert.equal(
      ...
      'completed',
      failureDiagnostic(result.terminal, failedObservations)
    );
...
class ScriptedProvider {
  ...
  async complete(request) {
    return { ...this.script.shift(), model: request.model, provider: this.id };
  }
}

function call(id, name, value) {
  return { id, type: 'function', name, input: { kind: 'json', value } };
}
function response(terminationReason, content, extra = {}) {
