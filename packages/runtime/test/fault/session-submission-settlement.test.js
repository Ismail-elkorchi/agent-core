...
for (const [timing, exitStatus, expectedResumes] of [
  ['before', 64, 1],
  ['after', 65, 0]
]) {
  test(`session submission settlement recovers exactly once after process death ${timing} commit`, async () => {
    ...
    try {
      const fixture = path.resolve(
        'packages/runtime/test/fault/fixtures/session-settlement-crash.mjs'
      );
      const crashed = spawnSync(process.execPath, [fixture, root, timing], {
        cwd: process.cwd(),
        encoding: 'utf8',
        timeout: 15_000
      });
        ...
        createRuntime() {
          return {
            resume(runId) {
              ...
              return control(
                runId,
                Promise.resolve({ state: 'ended', terminal: { runId }, deliveryDiagnostics: [] })
              );
            },
            run() {
              throw new Error('Recovered claimed work must resume its durable operation.');
            }
          };
        }
      });
