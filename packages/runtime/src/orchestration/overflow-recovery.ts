...
export type OverflowRecoveryResult =
  { kind: 'retry'; action: OverflowRecoveryAction } | { kind: 'diagnostic'; diagnostic: OverflowDiagnostic };

export const OVERFLOW_RECOVERY_STAGES = ['older_history', 'all_history'] as const;
export type OverflowRecoveryStage = (typeof OVERFLOW_RECOVERY_STAGES)[number];

export function createOverflowDiagnostic(
  estimate: RequestCostEstimate,
  actions: readonly OverflowRecoveryAction[]
): OverflowDiagnostic {
  return Object.freeze({
    reason:
      estimate.toolSchemaTokens > estimate.messageTokens + estimate.modelWindowTokens + estimate.contextTokens
        ? 'tool_schema_cost'
