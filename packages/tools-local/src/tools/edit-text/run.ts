...
export async function editText(
  ...
  if (failures.length > 0) {
    return {
      ...invalidToolInputObservation(
        'edit_text',
        `Text edit validation failed for ${String(failures.length)} item${failures.length === 1 ? '' : 's'}; no files were written.`,
        { failures }
      ),
      // Every source/range precondition is checked before entering the journal.
      execution: { state: 'not_started' }
    };
  }

  const summary = boundedDiffSummary(
    planned.flatMap((file) => file.diffLines),
    input.limits.maxDiffSummaryBytes
  );
  ...
  await checkpoint(context, {
    ...
    message: 'Text edit transaction planned.',
  ...
  await checkpoint(context, {
    ...
    message: `Text edit transaction ${transaction.outcome}.`
  });
...
export async function recoverEditText(
  ...
  if (input.dryRun)
    return {
      status: 'unavailable',
      reason: 'Dry-run text edits have no external mutation to reconcile.'
    };
  const capability = effect.intent.recovery;
  if (
    capability.kind !== 'buffered_mutation' ||
    capability.transactionId !== input.transactionId ||
    capability.reconcilerId !== 'agent-core.edit-text@1'
  ) {
    return {
      status: 'parameter_mismatch',
      reason: 'The text edit recovery capability does not match this transaction.'
    };
  }
  const root = requireRootedFileAuthority(context);
  const journal = requireToolService<TextPatchJournal>(
  ...
  if (capability.authority !== journal.recoveryIdentity)
    return { status: 'parameter_mismatch', reason: 'The text edit journal authority changed.' };
    ...
    async (authority) => {
      ...
      if (!receipt)
        return {
          ...
          reason: 'No durable text edit transaction receipt exists.'
        };
      ...
      if (!parsed.success || parsed.data.transactionId !== input.transactionId) {
        return {
          status: 'parameter_mismatch' as const,
          reason: 'The durable text edit receipt payload does not match this transaction.'
        };
      }
      if (receipt.result.outcome === 'rollback_failed') {
        return {
          status: 'unavailable' as const,
          reason: 'Text edit rollback is uncertain and requires explicit external reconciliation.'
        };
      }
      return {
        status: 'settled' as const,
        observation: observationFromPlan(parsed.data, false, receipt.result)
      };
    },
    context.signal
  );
...
async function planFile(
  ...
  | { ok: true; planned: PlannedFileEdit }
  | { ok: false; failures: { path: string; reason: string; message: string; editIndex?: number }[] }
> {
      ...
      failures: [
        {
          ...
          reason: inspected.failure.reason,
      ...
      failures: [
        {
          ...
          message: `Expected SHA-256 does not match the current file: ${file.path}`
        }
      ]
    };
      ...
      failures: [
        {
          ...
          message: `Edited file would contain ${String(newBytes)} bytes; the host maximum is ${String(limits.maxNewBytesPerFile)}.`
        }
      ]
    };
    ...
    scope: {
      ...
      ...(residue
        ? {
            ...
            omitted: { cleanup: transaction.cleanup.strandedPaths.length }
          }
        : {}),
      ...(applicationStatus === 'uncertain'
        ? {
            causes: ['rooted_file_state_uncertain'],
            omitted: { potentiallyAffectedPaths: potentiallyAffectedPaths.length }
          }
        : {}),
    ...
    .map((file) => ({
      ...
      resources: [
        rootedFileResource(file.path, {
          sha256: file.newSha256,
          fullSha256: file.newSha256,
      ...
      scope: {
        limits: {
          ...
          changedRanges: file.changedRanges.length,
...
function summarize(output: EditTextOutput): string {
  ...
  if (output.applicationStatus === 'no_change')
    return 'Text edit transaction completed with no content changes.';
  if (output.applicationStatus === 'applied')
    return `Applied ${String(output.diffSummary.totalChangedRanges)} localized text replacement${output.diffSummary.totalChangedRanges === 1 ? '' : 's'} across ${String(output.changedPaths.length)} file${output.changedPaths.length === 1 ? '' : 's'}.`;
  if (output.applicationStatus === 'not_applied')
    return 'Text edit transaction was rolled back; no requested content changes remain.';
...
export function editTransactionId(context: Pick<ToolExecutionContext, 'invocation'>): string {
  ...
  return `edit-${createHash('sha256')
    .update(
      [
        invocation.runId,
        invocation.turnId,
        invocation.toolBatchId,
        String(invocation.callIndex),
        String(invocation.toolAttempt)
      ].join('\0')
    )
    .digest('hex')}`;
}
