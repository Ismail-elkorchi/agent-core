...
export function adoptToolDefinition(tool: unknown): CompiledToolDefinition {
  ...
  for (const member of ['decodeInput', 'canonicalizeInput', 'snapshotInput', 'deriveEffects', 'invoke'] as const) if (typeof tool[member] !== 'function') throw new Error('Tool ' + name + ' ' + member + ' must be callable.');
